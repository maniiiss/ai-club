...
> GitPilot Desktop Design Mode 复用同一 Tauri + React + sidecar 生命周期，以独立 Design RPC 管理项目根目录中的 HTML/CSS/JS 原型；`.gitpilot/design.json` 是当前 manifest，`.gitpilot/revisions/` 保存不可变修订，`.gitpilot/project-guidelines.json` 保存项目级设计规范，`.gitpilot/sessions/<designId>/conversation.jsonl` 保存 Design 会话。React 只负责对话、设备预览与只读代码展示，预览 iframe 使用 sandbox 约束。Desktop 内置的 Design Preset Catalog 在构建期从 `gitpilot-desktop/src/design/presets/` 发现只读预设，将解析出的 Token 和 handoff 规则经既有 `design_save_guidelines` 落到项目级规范；预设 HTML 只作隔离预览，不进入项目文件树或新增后端 API。详细设计见 `docs/design-docs/gitpilot-desktop-design-mode-technical-design-v1.md`、`docs/design-docs/gitpilot-desktop-design-mode-multi-project-multi-file-technical-design-v1.md`、`docs/design-docs/gitpilot-desktop-design-mode-project-guidelines-technical-design-v1.md` 和 `docs/design-docs/gitpilot-desktop-design-preset-catalog-technical-design-v1.md`。

> GitPilot Desktop 的 Code、Work、Design 三种模式分别拥有工作目录边界：Code 由 Code session 管理项目 cwd，Work 由任务 ID 管理 `~/.gitpilot/agent/workspaces/<taskId>`，Design 由 Design store 管理用户选择的项目路径并由 sidecar 写入项目根目录及 `.gitpilot/`。Design 的 localStorage 只保存当前项目、页面/文件选择、视口、缩放、预览模式、预设等轻量 UI 状态；snapshot、文件正文、完整聊天记录、执行态和待办都不得进入 localStorage，恢复时必须通过 `design_open` 从项目目录和 sidecar 会话读取。Design Agent 可按修改规模选择增量 patch 或整文件替换，不强制分块插入；连续 patch 只传输 changed files，sidecar 只增量写入变更文件，预览请求做短暂 debounce。Design Agent 工具不要求模型传递或维护 `baseRevisionId`，sidecar 在每次 Design run 开始时固定正式 revision 作为并发保护基准，并在每次 patch/审批前由服务端注入；Desktop 直接发起的 patch 仍保留显式 revision 校验。通用工作台布局只接收当前模式显式传入的目录，不再隐式读取 Code 的 `currentProjectPath`；三种模式仅共享认证和模型运行时。
