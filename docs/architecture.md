...
- 智能体管理、模型管理、GitLab 管理、流水线中心、外部 Jenkins 兼容管理等平台能力配置
...
- GitLab、产品分支、AI Club Pipeline、外部 Jenkins、模型配置等平台能力管理
...
后端当前把流水线中心拆成“内置 AI Club Pipeline”和“外部 Jenkins 兼容”两条链路：
...
- `WoodpeckerApiService`：封装 Woodpecker API，统一处理 Bearer Token、运行历史和日志聚合
- `AiClubPipelineConfigTemplateService`：提供内置 Woodpecker 参数化模板，支撑缺失配置文件的表单化预览、手动 YAML 编辑和 MR 补全
- `CicdController`：新增 `/api/cicd/pipelines/**` 与 `/api/cicd/woodpecker/health`，原 Jenkins 接口继续保留
- `CicdManagementService.tryTriggerProjectPipeline`：默认优先触发启用的 AI Club Pipeline，再回退 legacy Jenkins binding

平台不再通过 AI Club 环境变量承载 Woodpecker 的 GitLab OAuth 或登录配置。Woodpecker 是部署级执行底座；业务用户不登录 Woodpecker，只使用 AI Club 平台登录、权限、运行历史和聚合日志。Woodpecker 运行时如果仍需要底座 forge 兼容配置，应放在 `.data/woodpecker/forge.env` 或服务器等价运行数据文件里，不进入 AI Club 平台配置。用户只在项目里创建流水线，并通过平台 API Token 让后端调用 Woodpecker 完成仓库同步和触发。若目标分支缺少 `.woodpecker.yml` 等配置文件，平台通过内置模板创建 GitLab MR 补全，不直接覆盖目标分支。前端入口改为“卡片总览 + 详情页”双层结构：列表页只保留卡片浏览与高频操作，运行历史和聚合日志统一进入详情页查看。补全弹窗默认展示模板参数表单，用户可填写项目根目录、推送服务器地址、镜像账号、SSH 私钥、触发分支和命令等元素；“部署到服务器”作为 Java / Node / Python / Docker 等模板的共享后置动作，通过开关启用，而不是拆成新的模板类型。后端按参数渲染 YAML，并把密码、私钥等敏感值写入 Woodpecker repo secrets。对于 monorepo 或多模块仓库，构建命令、Docker context 和部署产物路径都按“项目根目录”解析。高级用户仍可切换到手动 YAML 模式后提交自定义配置。
...
- 存储平台托管的 Hermes 用户会话记忆与项目 / Wiki 相关事实
- 提供会话相关的上下文检索能力
...
- 问答前，后端按当前用户 / 项目 / Wiki 作用域从 Hindsight 召回可直接塞进 Prompt 的记忆摘要
...
5. Hermes API Server 默认仅使用自身内置 memory；与 Hindsight 相关的外部记忆召回和用户会话沉淀都由后端负责
...
1. 用户在流水线中心创建 AI Club Pipeline，选择平台项目、GitLab 绑定、默认分支和 Woodpecker YAML 路径
...
4. 用户在流水线中心或首页快速构建触发流水线
5. 后端校验项目数据权限和 `cicd:build` 权限，并通过 GitLab API 确认目标分支存在配置文件路径
6. `WoodpeckerPipelineProvider` 使用平台 Token 调用 Woodpecker API
7. Woodpecker 返回 pipeline number、状态和链接，后端更新最近运行摘要并发送站内通知
8. 用户在流水线详情页查看运行历史或日志时，后端按需调用 Woodpecker API 聚合返回，不在本地落运行明细，也不跳转到 Woodpecker 登录入口

GitLab 自动合并的“合并后触发流水线”同样走 `tryTriggerProjectPipeline`：平台先找启用的 AI Club Pipeline；如项目尚未配置内置流水线，则继续兼容触发外部 Jenkins 绑定。
