...
- 平台对接以 Pi 内置 extension 形式实现（`src/extensions/gitpilot/`），随源码编译并默认加载，不侵入核心 TUI 与工具：
  ...
  - `platform-model`：注册 `gitpilot` provider，读取 `/api/cli/models` 启用 CHAT 模型；推理时用 `gpt_` 签发短期模型会话令牌（`gms_`，默认 900s），缓存并临近过期自动重建，通过 `streamSimple` 把请求改写到平台模型代理（OpenAI 走 `/chat/completions`、Anthropic 走 `/messages`）。平台模型 API Key、真实上游地址和完整请求审计留在 backend。
- backend `/api/cli/model-sessions/{id}` 只开放 Chat Completions 与 Anthropic Messages 两个固定协议路径，禁止把短期 session 变成任意 URL 代理；CLI Token scope 与设备授权流程保持不变。
