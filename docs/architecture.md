...
- 独立 API 管理菜单，通过 Yaade 嵌入式工作台按项目或公共空间维护接口资产
...
- `GitlabApiSyncService`：负责从后端仓库或混合仓库抽取 Spring 接口，并把平台生成的 API 项幂等同步到项目 Yaade collection

当前实现口径是：

- Yaade 是 API 资产唯一主数据源，平台不再把接口正文回写到 `project_api_*` 表
- 平台仍保留 `/api/apis/**` 与 `/api/projects/{projectId}/apis/**` 历史接口，但它们已经从主前端链路下线
- GitLab “同步 API”只对 `testProfileJson.repoKind` 为 `BACKEND` 或 `MIXED` 的绑定开放；同步时只覆盖带 `aiclubSync.source=GITLAB_SPRING_API` 标记的平台生成项，人工在 Yaade 中维护的请求保持不变
- 项目隔离依赖 Yaade collection 的 `groups` 字段，平台同步的是 group 名称而不是独立 group 实体
- iframe 请求不走平台 Bearer Token，而是走平台自己签发的 Yaade 代理会话 cookie
- `/apis` iframe 入口保留平台真实顶栏，并把嵌入态标记与当前平台主题 ID 通过 URL 参数和 `AI_CLUB_THEME_CHANGED` 消息传给 Yaade；Yaade 独立容器的品牌补丁在嵌入态隐藏自身原生顶栏，把登录页改为先等待平台代理单点登录确认，并把平台主题映射为自身浅色工作台背景和强调色变量
...
1. 用户进入独立菜单 `API 管理`
2. 前端加载可选项目列表、Yaade 健康状态，并默认进入 `未关联项目` 公共空间
3. 前端调用 `POST /api/yaade/embed-sessions`，后端先确保当前项目对应的 Yaade collection 已存在
4. 后端按当前用户可见项目集合，同步 Yaade 本地账号和 group membership
5. 后端以该用户身份登录 Yaade，把远端 session 换成平台自己的 HttpOnly 代理 cookie
6. 前端 iframe 加载 `/api/yaade/proxy/`，并把嵌入态标记和当前平台主题 ID 传给 Yaade；后续静态资源和 API 请求都由平台后端代发到独立部署的 Yaade
7. 用户在 Yaade 中录入或维护接口资产，Yaade 自身保存 collection、request、environment 等内容
8. 若远端 session 失效，平台代理层会按用户绑定自动重新登录一次；若 Yaade 不可用，前端仅在 `/apis` 页面展示错误态
...
4. 后端调用 `code-processing` 的 `/api/code/gitlab-spring-apis/extract`，由代码处理服务 clone/fetch 仓库并抽取 Spring Controller、参数、DTO 字段和枚举注释
5. 后端确保项目 Yaade collection 存在，并调用 Yaade request CRUD 创建、更新或删除平台生成项
