...
- 独立 API 管理菜单，通过 Yaade 嵌入式工作台按项目维护接口资产
...
- `/apis` iframe 入口保留平台真实顶栏，并把嵌入态标记与当前平台主题 ID 通过 URL 参数和 `AI_CLUB_THEME_CHANGED` 消息传给 Yaade；平台还会通过 `AI_CLUB_PROJECT_CONTEXT` 传递当前项目与可见项目映射，Yaade 通过 `AI_CLUB_PROJECT_CHANGED` 回传项目切换；Yaade 独立容器的品牌补丁在嵌入态隐藏自身原生顶栏，把登录页改为先等待平台代理单点登录确认，并把平台主题映射为自身浅色工作台背景和强调色变量
...
1. 用户进入独立菜单 `API 管理`
2. 前端加载当前用户可见项目列表与 Yaade 健康状态，并确定默认项目
3. 前端调用 `POST /api/yaade/embed-sessions`，后端确保当前项目以及其它可见项目的 Yaade collection 绑定存在
4. 后端按当前用户可见项目集合，同步 Yaade 本地账号和 group membership
5. 后端以该用户身份登录 Yaade，把远端 session 换成平台自己的 HttpOnly 代理 cookie
6. 前端 iframe 加载 `/api/yaade/proxy/`，并把嵌入态标记、当前平台主题 ID、当前项目与可见项目上下文传给 Yaade；后续静态资源和 API 请求都由平台后端代发到独立部署的 Yaade
7. 用户先在 Yaade 内部顶部项目下拉选择项目，再在左侧树中选择该项目下的集合或接口；Yaade 切换项目时只回传 `projectId` 给平台路由，不重建 iframe
8. 用户在 Yaade 中录入或维护接口资产，Yaade 自身保存 collection、request、environment 等内容；集合归属在嵌入态收口为单项目选择，不再直接暴露原始 group 字符串
9. 若远端 session 失效，平台代理层会按用户绑定自动重新登录一次；若 Yaade 不可用，前端仅在 `/apis` 页面展示错误态
...
6. 前端展示新增、更新、删除、跳过数量，并可跳转到 `/apis?projectId=...` 查看结果
