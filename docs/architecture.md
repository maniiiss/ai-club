...
管理端和公众端共享 `deep-sea`、`ocean-mist`、`signal-teal` 三个稳定主题 ID。两套前端各自保留本地 `gitpilot-theme` 缓存，仅用于首次渲染时减少闪烁；登录成功或恢复会话后，服务端返回的 `CurrentUserInfo.themeId` 会覆盖本地视觉状态，退出登录统一恢复 `deep-sea`。
