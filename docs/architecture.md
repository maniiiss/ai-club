...
#### 3.2.3 GitLab 仓库代码结构快照子系统

后端当前新增了 GitLab 仓库代码结构快照子系统，职责边界如下：

- `gitlab_code_structure_snapshot`：按 `binding + branch` 保存最近一次 GitNexus 仓库概览快照
- `GitlabController`：对前端暴露代码结构查询、后台刷新和局部查询接口
- `GitlabManagementService`：负责分支解析、数据权限校验、快照状态流转和后台刷新调度
- `GitlabCodeStructureClientService`：负责调用 `code-processing` 内部 GitNexus 结构化接口
...
3. 后端按 `binding + branch` 返回最近一次结构化快照
4. 用户手动触发刷新时，后端把快照状态切到 `BUILDING` 并异步调用 `code-processing`
5. `code-processing` 在稳定缓存工作区里 clone/fetch 仓库、执行 `gitnexus analyze`
6. `code-processing` 再执行固定概览 query，并为 Top 符号补 `context`
7. `code-processing` 返回概览 JSON / 图谱 JSON / Markdown 摘要
8. 后端更新快照状态为 `READY`、`DEGRADED` 或 `FAILED`
9. 用户输入关键词时，前端调用局部查询接口；后端继续通过 `code-processing` 复用当前分支缓存和 `.gitnexus` 索引返回临时子图
