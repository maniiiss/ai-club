...
- 独立 API 管理菜单，以及按项目或未关联空间维护接口文档、环境和调试记录
...
后端当前新增了独立的 API 管理子系统，职责边界如下：

- `project_api_profile`：保存 API 文档标题、说明和版本号，可按项目或未关联空间归属
- `project_api_folder`：保存接口目录树
- `project_api_endpoint`：保存接口定义、参数表、请求体示例、响应示例和默认调试配置
- `project_api_environment`：保存 API 环境、环境变量和鉴权配置
- `project_api_debug_record`：沉淀最近调试记录的脱敏请求/响应快照
- `ApiManagementController`：对独立菜单页面暴露目录、接口、环境、OpenAPI 导入导出与调试接口
- `ProjectApiController`：对前端暴露目录、接口、环境、OpenAPI 导入导出与调试接口
- `ProjectApiManagementService`：负责项目可选关联、JSON 结构校验、OpenAPI 映射、HTTP 代理调试与脱敏落库

第一版实现口径是：

- 只支持 REST HTTP，不支持 GraphQL、WebSocket 或文件上传调试
- 请求/响应文档采用“参数表 + 示例文本”，不做完整 Schema 编辑器
- OpenAPI 导入按 `method + path` 做当前 API 空间内增量更新
- 调试固定走 backend 代理；若绑定项目则复用项目级数据权限，否则只校验 API 管理权限
- 文本响应保存完整脱敏内容，二进制响应只保存类型、大小和摘要
...
1. 用户进入独立菜单 `API 管理`
2. 前端先加载可选项目列表，并默认进入 `未关联项目` 空间
3. 用户切换到某个项目空间或保持在未关联空间后，前端读取该空间的 API profile、目录树和环境列表
4. 用户选择某个接口后，前端再读取接口详情
5. 用户在 `编辑` 标签中维护参数表、请求体示例、响应示例和默认调试环境
6. 用户在 `调试` 标签中选择当前空间的环境并覆盖本次请求值
7. 后端把接口默认定义、调试覆盖值、环境变量和鉴权配置合并成最终请求
8. 后端通过 Java `HttpClient` 发起真实请求，并把脱敏后的请求/响应快照保存到当前空间的调试记录
9. 用户可随时导入 OpenAPI 文档初始化当前空间的接口资产，或把当前空间全部接口导出回 OpenAPI

这条链路属于平台独立的“API 资产中心”，当前不依赖 `code-processing`。
