...
GitPilot CLI 本地 Pi MVP 已形成独立的本地执行平面：

- `gitpilot-cli` 运行在用户设备上，嵌入 Pi Agent Core，并只注册仓库范围内的文件、Git 和 Shell 工具。
- `packages/gitpilot-agent-core` 是 `gitpilot-cli` 与 `pi-runtime` 共用的 Pi Agent 初始化、模型解析、历史转换和事件归一化边界。
- CLI 使用设备授权换取独立 CLI Token；Token 只保存在系统凭据库，服务端数据库只保存 hash。
