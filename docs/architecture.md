...
Hermes 还提供用户级“个人文件库”。用户在管理端或公众端的 Hermes“知识”面板上传 `.pdf/.docx/.pptx/.xlsx` 后，后端复用通用文档资产与 Markdown 转换链路保存原始文件和 Markdown，再切块、生成 embedding 并写入 Qdrant collection `hermes_file_library_chunks`。文件库是个人长期知识，不替代项目 Wiki，也不产生项目共享文件库。
...
3. 后端根据当前用户、页面路由、上下文对象、已绑定对象和结构化 `slashCommand` 组装提示词，并先从 Hindsight 召回当前用户自己的会话记忆，再从 Qdrant 召回个人文件库证据和 Wiki 知识证据
...
- `/wiki` 启用 Wiki 问答 Skill
- `/需求` 启用工作项创建 Skill
- `/仓库扫描` 启用仓库扫描 Skill
- `/执行任务` 启用执行任务查询 / 摘要 Skill
