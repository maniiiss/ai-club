...
其中 GitLab 自动合并的 MR 审查现在额外承担“历史问题带入”职责：backend 会按“GitLab 项目标识快照 + MR IID”读取最近一次 `AI_REJECTED` 日志里的结构化问题清单，通过 `/api/code/review` 一并传给 `code-processing`；`code-processing` 需要返回当前仍需处理的问题总表、已修复历史问题和未修复历史问题，backend 再据此决定是否放行自动合并，并把结果固化到自动合并日志。
