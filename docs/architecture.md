...
6. 前端 iframe 加载 `/api/yaade/proxy/`，后续静态资源和 API 请求都由平台后端代发到独立部署的 Yaade
...
其中 Gitee 的全局 `enterpriseId` / `accessToken` 已不再要求项目级重复录入，而是统一走“系统设置-环境变量管理”：

- 优先读取后台保存的运行时覆盖配置
- 若后台未配置，则回退 `.env` / Spring 配置
- 若历史数据仍存在项目绑定快照，则作为最后一层 legacy 兜底
