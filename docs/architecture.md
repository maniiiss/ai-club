...
> Design 实时渲染补充：正式场景仍由 `.gitpilot/design/<designId>/design.json` 提供，active run 的事务先进入 `.gitpilot/design/<designId>/drafts/<runId>/operations.jsonl` 与 checkpoint，Desktop 通过 RAF 合并 `design_patch_applied` 后由 CanvasKit 绘制；重连时 `design_open` 可一次性返回 `draftSnapshot`，但 canonical 与 draft 仍在 Desktop store 分层保存；completed 或 interrupted settled 才写入正式 revision。orphaned draft 只能经 `design_recover_draft` 明确保留或放弃，localStorage 不保存 draft 正文。

> Design 低延迟补充：首批 Agent patch 控制在 4～8 个骨架操作，Design 会话在新回合前按消息数量和序列化大小裁剪并追加轻量场景摘要，避免历史思考拖慢首 token；模型等待真实 patch 时，CanvasKit 只在 transient 层显示 RAF 驱动的短轨迹和笔尖，动画不进入场景、撤销或 journal，窗口隐藏时暂停非必要帧。

> GitPilot Work 使用 GitPilot agent 根目录下的独立任务 workspace、持久化 AgentSession 与文件产出；Code 继续使用项目 cwd 和 Code session。Work 内置 `office-docx`、`office-xlsx`、`office-pptx` Skill，通过无 Shell、无网络的受控工具在任务目录生成并检查真实可编辑 Office 文件，复杂精修交给用户本机 Office/WPS。公众端项目、工作项、评论和附件通过 CLI 内置插件按需访问，写操作经 Desktop 确认并携带 Work 任务标识审计。详细设计见 `docs/design-docs/gitpilot-work-collaboration-technical-design-v1.md` 和 `docs/design-docs/gitpilot-work-office-skills-technical-design-v1.md`。

> GitPilot Desktop 现包含隔离的 `gitpilot-code` 与 `gitpilot-work` 模式：Code 保持项目编码 Agent 链路；Work 的任务、对话与成果只在 Desktop IndexedDB 保存。Work 的联网研究经 sidecar 调用受认证的 `/api/cli/work/research`，由后端托管搜索密钥、限流与结果裁剪，Desktop 不获得任意网络、Shell、Git 或项目文件权限。Work 对话与 Code 一致展示执行过程：sidecar 把思考增量、工具生命周期与正文段收口转发为 `work_*` 流事件，Desktop 用纯函数状态机（`work-execution.ts`）按“正文 → 执行过程 → 正文”真实顺序归并，执行批次以 `kind === 'execution'` 的 WorkMessage 随 IndexedDB 持久化并复用 Code 的执行批次卡片回显。详见 `docs/design-docs/gitpilot-work-technical-design-v1.md`。

> GitPilot Desktop Design Mode 复用同一 Tauri + React + sidecar 生命周期，以独立 Design RPC 管理项目根目录中的 `CanvasDesignDocument` 原生场景；`.gitpilot/design/<designId>/design.json` 是当前场景，`revisions/` 保存不可变修订，`project-guidelines.json` 保存项目级设计规范，Design 会话保存在原生工作区的 `.session/conversation.jsonl`。CanvasKit 是设计内容的唯一渲染器，React 只负责工具栏、页面/图层树、Inspector、对话和无障碍辅助界面；Design Agent 通过受校验的 Canvas 事务产生 draft patch，sidecar 在 journal 前把旧版 `rectangle/fill/字符串 text` 归一化为可绘制 canonical 节点，Desktop 对旧 sidecar 快照保留同一兼容层；图标使用可序列化的 `type: "icon"` + `icon.name` 语义节点，由 `canvas-icons.ts` 解析为内置 24×24 矢量路径或 custom SVG path，不能只把图标名写成普通文本；Desktop 以 RAF 合并事件并实时绘制，`design_run_settled` 才收口正式 revision。Agent 首屏采用“首批骨架 + 最多两轮细节”的低延迟批次策略，Design 默认不加载 Web 搜索工具且将 medium/high 思考预算压到 low，显式需要外部参考时再开启对应工具。Desktop 内置的 Design Preset Catalog 在构建期从 `gitpilot-desktop/src/design/presets/` 发现只读预设，将解析出的 Token 和 handoff 规则经既有 `design_save_guidelines` 落到项目级规范。详细设计见 `docs/design-docs/gitpilot-desktop-design-mode-technical-design-v1.md`、`docs/design-docs/gitpilot-desktop-design-mode-streaming-technical-design-v2.md`、`docs/design-docs/gitpilot-desktop-design-canvaskit-native-renderer-technical-design-v1.md`、`docs/design-docs/gitpilot-desktop-design-mode-live-render-technical-design-v1.md`、`docs/design-docs/gitpilot-desktop-design-mode-multi-project-multi-file-technical-design-v1.md` 和 `docs/design-docs/gitpilot-desktop-design-mode-project-guidelines-technical-design-v1.md`。

> GitPilot Desktop 的 Code、Work、Design 三种模式分别拥有工作目录边界：Code 由 Code session 管理项目 cwd，Work 由任务 ID 管理 `~/.gitpilot/agent/workspaces/<taskId>`，Design 由 Design store 管理用户选择的项目路径并由 sidecar 写入项目根目录及 `.gitpilot/`。Design 的 localStorage 只保存当前项目、页面/图层选择、视口、缩放、预览模式、预设等轻量 UI 状态；snapshot、场景正文、完整聊天记录、执行态和待办都不得进入 localStorage，恢复时必须通过 `design_open` 从项目目录和 sidecar 会话读取。Design Agent 只提交受校验的 Canvas 场景事务，运行中 patch 进入独立 draft 并通过 `design_patch_applied` 增量传输，Desktop 按 RAF 合并后由 CanvasKit 实时绘制；`design_run_settled` 才创建正式 revision。手工拖动和自由路径使用 transient 几何，pointerup 时提交一个用户事务；AI 运行期间的结构性手工修改默认排队，避免多个作者同时重排节点树。Design Agent 工具不要求模型传递或维护 `baseRevisionId`，sidecar 在每次 Design run 开始时固定正式 revision 作为并发保护基准，并在每次 patch/审批前由服务端注入；Desktop 直接发起的 patch 仍保留显式 revision 校验。通用工作台布局只接收当前模式显式传入的目录，不再隐式读取 Code 的 `currentProjectPath`；三种模式仅共享认证和模型运行时。
...
> GitPilot 将 `cross-agent-harness`、`gitnexus`、`office-docx`、`office-xlsx`、`office-pptx` 作为随 sidecar 发布的内置 Skill：资源在 `gitpilot-cli/src/bundled-skills/` 维护，Node 构建复制到 `dist/`，Bun sidecar 复制到 Tauri `resources/skills/`。每位用户首次创建 AgentSession 服务时，平台仅在 `~/.gitpilot/agent/skills/<name>/` 不存在时复制完整 Skill 包；已有用户同名目录永不覆盖，并继续由既有资源加载器发现。默认模式：`cross-agent-harness` 与 `gitnexus` 进 Code（gitnexus 指导模型经 GitNexus MCP 知识图谱做代码理解、影响分析与重构护航），Office 三件套进 Work。GitNexus MCP 本身不随平台分发：Desktop MCP 管理页提供“推荐服务”预设，用户一键写入全局 `mcp.json`（Windows 经 `cmd /c npx` 启动，其他平台直接 `npx`，默认仅授权 Code），依赖本机 Node.js 20+ 与项目级 `npx gitnexus analyze` 索引。

> Code 任务的“已编辑文件”采用任务级工作区收口链路：`AgentSession` 在 `_runAgentPrompt` 开始时用临时 `GIT_INDEX_FILE` 对当前工作区执行 `git add -A` 和 `git write-tree`，形成不改变用户真实 index 的 baseline；执行期间只记录工具参数中声明的文件路径，`agent_settled` 时再生成 final tree，并以 `git diff baselineTree finalTree -- <task paths>` 计算最终净 diff。这样任务开始前已有的 tracked/untracked 改动和其它任务修改的未触及文件不会被带入；两个任务同时修改同一个文件的精确归因暂不处理。最终 diff 通过 RPC 发送给 Desktop，覆盖工具级中间统计；持久化 JSONL 仅保存压缩 artifact 的引用、hash 和统计，baseline/final 临时 index 在收口后清理。非 Git 工作区或 Git 收口失败时保留 Desktop 原有工具级降级链路。
...
- GitPilot/Assistant 抽屉式对话入口及动作卡片消费
...
- 服务器管理
- 系统管理：用户、角色、权限、环境变量管理、操作日志、版本发布
...
- 个人资料设置与主题切换
- 新手引导与重播入口，按 `dashboard`、`projects`、`chat`、`development`、`project-detail`、`planning`、`knowledge`、`testing`、`test-plan-detail`、`execution`、`execution-task-detail`、`assistant` 共 12 个 page key 记录用户已完成页面；`assistant` 引导在项目助手抽屉首次打开时触发，其余随页面加载自动启动
...
公众端新手引导由 `frontend-public/src/components/guide/guideSteps.ts` 维护唯一 page key 与步骤配置，`useGuide` 在页面关键 DOM 渲染后调用 Driver.js 展示遮罩式引导。完成、跳过或倒计时关闭都会通过 `PUT /api/auth/guide-status` 全量覆盖当前用户的 `guideCompleted`，后端 `AuthService` 只允许 `dashboard`、`projects`、`chat`、`development`、`project-detail`、`planning`、`knowledge`、`testing`、`test-plan-detail`、`execution`、`execution-task-detail`、`assistant` 写入 `user_info.guide_completed`，登录态中的 `CurrentUserInfo.guideCompleted` 同步刷新。page key 白名单与前端 `GUIDE_PAGE_KEYS` 必须一一对应，新增引导页面时两处同步修改。`assistant` 引导挂载在 `ProjectLayout`，因工作区随抽屉懒挂载，引导在助手抽屉首次打开时启动，避免与各子页面引导叠加。公众端顶部用户菜单提供“重播新手引导”入口，重置完成状态后回到 `/dashboard` 重新触发引导。
...
DataWorkbench 的下一阶段是独立的 DataQuery 语义查询能力：平台库维护动态 PostgreSQL 数据源注册、Schema 快照、同义词、概念、定义、枚举字典和行列级策略；后端将自然语言受限表达归一为 `SemanticQueryDsl`，再按已发布语义版本编译参数化 PostgreSQL `SELECT` 并在只读事务中执行。模型只能参与受限表达和脱敏结果总结，不能生成或执行 SQL；DataQuery 与 DataChange 使用独立 DSL，读取结果不能直接绕过写入审批链路。完整方案见 `docs/design-docs/data-workbench-semantic-query-technical-design-v1.md`。
...
- `ModelUsageStatsController`（`/api/model-usage-stats`，权限 `system:model-usage:view`）：`getOverview / getByModel / getByUser / getTrend / getByProvider`，模型聚合键为 `(model_name, provider)`（`COALESCE` 空值归 `<unknown>`）；`getByModel` 额外返回关联的 `ai_model_config.name` 供调用量排行展示，未关联配置时前端回退实际模型名；用户 Token 用量单独按 `user_id` 聚合，覆盖表内模型、env 模型与回传模型。
- 前端 `/model-usage-stats`（`ModelUsageStatsView`）：引入 ECharts（按需注册）做模型调用量排行、Token 分布、调用趋势可视化；模型明细不混入用户列，筛选范围下的用户 Token 用量在独立表格展示，与 `/agent-usage-stats`（按智能体/用户维度）互补。
...
Provider 的抽象边界包含用户/共享 scope、语义召回、记忆文档写入、删除和 consolidation 操作；Hindsight 的 bank 映射、HTTP DTO 转换和数据库回退均封装在 `HindsightMemoryProvider` 内。未来替换 Redis、pgvector 或其他记忆服务时，只需新增 Provider 实现，不需要修改 GitPilot 对话编排。Runtime 只消费 backend 注入的记忆上下文，不直接访问任何记忆存储。
...
### 4.12 GitPilot 单条回答用户反馈闭环

GitPilot 反馈与通用的“反馈与建议”入口分离，按助手消息建立可运营、可复盘的闭环：

1. 公众端在已持久化的助手回答下提供点赞/点踩。点踩通过预置原因和可选补充说明采集结构化信号；流式中、失败或尚未获得数据库消息 ID 的回答不展示评价控件。
2. `assistant_message_feedback` 保存会话、用户消息、助手消息、提交用户快照、问题/回答快照、Runtime/路由/项目上下文、评价方向、运营状态、处理结论和数据集状态。同一用户对同一助手消息使用唯一约束幂等覆盖。
3. 点赞反馈默认进入 `AUTO_CLOSED`；点踩反馈进入 `NEW`，管理员通过 `/api/assistant-feedback` 队列完成分诊、负责人分配、处理结论和改进标签。状态变化写入 `assistant_feedback_activity`，便于审计和运营回溯。
4. 用户通过 `/api/assistant/feedback` 查询自己的反馈和处理结果；管理员通过 `system:assistant-feedback:view` / `system:assistant-feedback:manage` 权限访问运营页面。纳入 `INCLUDED` 的样本通过数据集接口提供给后续评测、提示词复盘或人工分析，不在本链路中自动修改模型行为。
5. 助手流式 `done` 与非流式响应都返回本轮持久化的 `sessionId`、`userMessageId`、`assistantMessageId`，前端不使用临时渲染 ID 关联反馈，兼容历史会话仍按详情接口读取真实消息 ID。

反馈表和通用 `user_feedback` 表保持独立：前者服务于回答质量和智能体复盘，后者继续承载产品建议、体验反馈和非回答类问题。
...
- GitPilot 内置 `pi-web-access@0.22.0` 与 `pi-mcp-adapter@2.21.0`：Web 默认覆盖 Code、Work，Design 为降低首轮上下文和生成延迟默认不加载 Web 搜索工具，只有显式需要外部参考时才开启；MCP 服务按独立的 `mcp-scopes.json` 分配三种模式，未分配服务默认仅 Code。标准连接定义仍保留在全局与项目 MCP 配置层，凭据和 OAuth 不进入 Desktop 状态；Bun sidecar 静态打包两个扩展及运行依赖。详细边界见 `docs/design-docs/gitpilot-web-mcp-extensions-technical-design-v1.md`。
...
- Desktop code 模式右侧工具窗口新增“审查”页签：每轮任务收敛后，聊天流的 changed_files 虚拟消息（由 `changed-files.ts` 从实时 ExecutionStep 或持久化 toolCall/toolResult 聚合的 unified diff）经 ChatView 派生写入 `useReviewStore`，审查面板与聊天“已编辑 N 个文件”卡片同源展示本轮改动文件；点击卡片中的文件行会展开右侧栏、激活审查页签并展开该文件 diff。该页签纯前端派生（不持久化、不新增 RPC），与规划中的 Git Review Workbench（Git 快照级 AI 审查）边界清晰：前者只回放工具级编辑 diff，后者仍按既定设计走 sidecar `RepositoryService` 与 backend 审查编排。
- Code 模式右侧栏 Git 面板（v2）在“提交并推送”链路上自动跳过误跟踪文件：sidecar `RepositoryService.readState` 用 `git ls-files --cached --ignored --exclude-standard` 标注 `ignoredTracked`，这类文件的改动不进入自动暂存路径（用户手动暂存的保留），面板单独“被忽略的已跟踪”分组展示并提供 `git_untrack_paths` 解除跟踪（`git rm --cached` 保留本地文件，提交删除后忽略规则才生效）；`ignoredTracked` 为可选字段，旧 sidecar 混布时新 UI 自然降级。详细设计见 `docs/design-docs/gitpilot-desktop-code-git-panel-technical-design-v2.md`。
