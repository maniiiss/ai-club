...
- 独立 API 管理菜单先展示当前用户可见项目 GROUP，再进入指定项目的 Yaade 嵌入式工作台维护接口资产
...
公众端积分系统是后续公众端 AI 能力的统一扣费底座，第一版只提供账户、配置、调账、流水与服务级扣减能力，不在前端暴露任何直接消费接口。

核心数据由后端 `CreditService` 统一维护：

- `credit_global_config` 保存注册赠送积分等全局开关，新注册用户在 `AuthService.register` 创建账号后自动开户，并按当前配置赠送积分
- `credit_feature_config` 保存 AI 功能扣费规则，后续业务功能通过 `featureCode` 获取每次固定扣减积分
...
管理端通过 `/api/credits/config`、`/api/credits/features`、`/api/credits/accounts` 维护全局赠送、功能扣费规则和用户余额，查看权限为 `system:credit:view`，配置与调账权限为 `system:credit:manage`。公众端只调用 `/api/credits/me` 和 `/api/credits/me/transactions` 展示当前用户余额与个人流水。

未来公众端 AI 功能接入时，不应由前端传入扣减数量或直接调用扣减接口，而是由对应后端业务服务调用 `CreditConsumptionService.consumeForFeature(userId, featureCode, businessKey, reason, supplier)`。该服务先校验功能启用与余额，扣减成功后执行实际 AI 业务；业务成功时保留消费流水，重复 `businessKey` 不重复扣费；业务抛出运行时异常时自动写入退款流水并恢复余额，余额不足时直接拒绝且不写消费流水。
