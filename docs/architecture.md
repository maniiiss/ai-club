...
- Hermes 对话服务地址、API Key、模型名、请求超时，以及语音转写服务地址、API Key、模型名和超时时间
- Hindsight API 地址、API Key、bank 前缀、recall 预算和请求超时

这些配置在 `.env.example`、`.env.server.example` 与 `application.yml` 中的初始值保持为空，示例文件只承担用途说明和本地启动回退入口。未纳入后台管理的数据库、Redis、MinIO、内部服务令牌、GitNexus 端口、Hindsight 自身 LLM / Embedding / Reranker provider 等仍按部署配置文件管理，后续如果需要运行时切换再单独纳入注册表。
