...
8. 用户在统一详情页查看运行历史或日志时，后端按 provider 调用 Woodpecker 或 Jenkins API 返回，不在本地落运行明细，也不跳转到对应平台登录入口
