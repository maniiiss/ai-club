...
- `rabbitmq`：异步任务消息队列，当前承载聊天室 Agent 任务投递、延迟重试和 DLQ
...
- `rabbitmq`：承载异步任务信号、消费者并发、ack/nack、延迟重试和死信队列；聊天室 Agent 以数据库任务表为事实源，RabbitMQ 只投递轻量 `taskId`
