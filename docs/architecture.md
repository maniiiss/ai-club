...
固定步骤为 `CODE_CONTEXT -> DESIGN_DRAFT -> DESIGN_REVIEW`。每一步只能绑定启用的 `AGENT_RUNTIME + CODEX_CLI/CLAUDE_CODE_CLI`；绑定由管理员发布的执行编排预设提供，发起人只选择明确的 GitLab binding 和目标分支。创建时同时固化编排版本和实际 Agent 快照。`code-processing` 将三步映射到 `TECHNICAL_DESIGN` 模式：Codex 使用 read-only sandbox，Claude 使用 plan 权限并仅开放 Read/Grep/Glob/LS，从命令层禁止代码编辑。
...
`DEVELOPMENT_IMPLEMENTATION` 与 `TECHNICAL_DESIGN_AUTHORING` 是首批受管场景。后端代码目录定义固定逻辑步骤和 Agent 兼容规则；管理端“执行中心 / 编排管理”只允许管理员为固定步骤选择 Agent 和设置超时，不允许改变顺序、增删步骤或构造自由 DAG。
