...
- 独立 API 管理菜单当前先展示用户可见项目 GROUP，再进入指定项目的 Yaade 嵌入式工作台；2026-06-24 已确认后续以原生 API 工作台替代 Yaade，设计见 `docs/api-studio-native-technical-design-v1.md`
...
详细产品信息架构与分阶段设计见 `docs/public-saas-frontend-technical-design-v1.md`。
...
后端当前把 API 管理切换为 `Yaade` 嵌入式子系统，职责边界如下：

- `platform_yaade_project_binding`：保存平台项目与 Yaade 顶级 collection 的映射、归档状态和最近同步时间
- `platform_yaade_user_binding`：保存平台用户与 Yaade 本地账号的映射，以及平台代持的随机密码密文
- `YaadeController`：对前端暴露嵌入会话创建、健康检查、项目绑定查询与修复同步接口
- `YaadeProxyController`：代理 Yaade Web UI、静态资源与后续 API 请求，屏蔽浏览器侧对 Yaade 原始地址的直接访问
- `YaadeProjectSyncService`：负责项目到顶级 collection 的创建、改名、归档和幂等修复
- `YaadeUserSyncService`：负责平台用户账号同步、group 收敛和平台代登登录恢复
- `YaadeEmbedSessionService`：负责创建平台侧代理 cookie、续租远端 session 与登出清理
- `YaadeApiCatalogService`：负责按项目 Yaade 根 collection 拉平 REST request，供平台侧 AI 测试用例生成选择单接口
- `ApiTestCaseAiService`：负责读取单个 Yaade request、脱敏接口上下文并调用对话模型生成结构化测试用例建议
- `GitlabApiSyncService`：负责从后端仓库或混合仓库抽取 Spring 接口，并把平台生成的 API 项按 Controller 目录幂等同步到项目 Yaade collection 树

当前实现口径是：

- Yaade 是 API 资产唯一主数据源，平台不再把接口正文回写到 `project_api_*` 表
- AI Club 在 Yaade iframe 外层提供单接口 AI 测试用例生成增强层；该能力只读取 Yaade request 并生成可复制建议，不保存、不执行、不写回 Yaade
- 平台仍保留 `/api/apis/**` 与 `/api/projects/{projectId}/apis/**` 历史接口，但它们已经从主前端链路下线
- GitLab “同步 API”只对 `testProfileJson.repoKind` 为 `BACKEND` 或 `MIXED` 的绑定开放；新生成接口写入项目根 collection 下的 Controller 子目录，同步时只覆盖带 `aiclubSync.source=GITLAB_SPRING_API` 标记的平台生成项，人工在 Yaade 中维护的请求保持不变
- 项目隔离依赖 Yaade collection 的 `groups` 字段，平台同步的是 group 名称而不是独立 group 实体
- iframe 请求不走平台 Bearer Token，而是走平台自己签发的 Yaade 代理会话 cookie
- `/apis` 是 API 项目 GROUP 列表入口，只读取项目与 Yaade 绑定摘要，不创建嵌入会话；`/apis/projects/{projectId}` 承载 iframe 工作台，平台顶栏提供“返回 API 项目”入口，同时把嵌入态标记与当前平台主题 ID 通过 URL 参数和 `AI_CLUB_THEME_CHANGED` 消息传给 Yaade；平台还会通过 `AI_CLUB_PROJECT_CONTEXT` 只传递当前项目映射，Yaade 侧据此过滤当前项目集合；Yaade 独立容器的品牌补丁在嵌入态隐藏自身原生顶栏，把登录页改为先等待平台代理单点登录确认，并把平台主题映射为自身浅色工作台背景和强调色变量

2026-06-24 已确认下一阶段不再沿用“保留 Yaade 作为底层能力层”的方案，而是建设平台原生 API 工作台：

- 新主数据源为平台业务库中的 `api_studio_*` 表，第一版不迁移 Yaade 数据，也不复用旧 `project_api_*` 表
- 原生工作台复用平台项目，结构为“项目 -> 多级目录 -> API”，覆盖 REST/HTTP 资产管理、项目级环境、结构化入参出参、后端代理调试、个人调试记录和 API 版本快照
- Yaade 仅作为过渡期 legacy 入口保留；待原生工作台达到闭环后，逐步删除 Yaade iframe、代理会话、构建补丁、独立 compose 和相关配置
- 详细设计见 `docs/api-studio-native-technical-design-v1.md`

#### 3.2.5.1 原生 API 工作台落地（api-studio）

按 `docs/api-studio-native-technical-design-v1.md` 第一版实现已落地，组件清单如下：
...
详细设计见 `docs/pipeline-woodpecker-provider-technical-design-v1.md` 与 `docs/pipeline-woodpecker-provider-technical-design-v2.md`。
...
Qdrant 是 Wiki 知识检索的专用向量后端，当前通过 Docker 启动，主要用于：
...
- `Qdrant`：Wiki 知识 chunk、相关页面候选、Hermes 的 Wiki 证据召回
- 业务库：Wiki 原文、目录结构、页面权限、同步状态、版本历史

Wiki 页面保存后，不再把整页内容直接 retain 到 Hindsight bank，而是通过后台同步任务切块、生成向量，并写入 Qdrant 的项目/空间 collection。搜索时先从业务库拿关键词候选，再从 Qdrant 拿向量候选，最后由专用 reranker 重排，保证“术语精确匹配”和“自然语言语义命中”同时可用。
...
- `qdrant`：保存 Wiki 知识索引向量与片段 payload
...
- `credit_feature_config` 保存 AI 功能扣费规则，业务功能通过 `featureCode` 获取每次固定扣减积分。已配置的功能编码包括：`REQUIREMENT_AI`（需求 AI 助手：标准化需求、拆解子任务）和 `TEST_CASE_AI`（测试用例生成）
...
自动合并 AI 审核的积分扣费接入方案、扣费粒度、前端确认交互与后端变更明细见 `docs/public-credit-technical-design-v1.md`。
...
3. 后端根据当前用户、页面路由、上下文对象、已绑定对象组装提示词，并优先从 Hindsight 召回当前用户自己的会话记忆，再按 Wiki 场景从 Qdrant 召回知识证据
...
这条链路与 4.8 节的"模型 Token 计量"不耦合：benchmark 调用始终走独立路径，usage 仅用于指标统计，不进入业务计量事件表。详细数据模型与字段约束见 `docs/model-benchmark-technical-design-v1.md`。
...
后端的“系统设置-环境变量管理”采用固定注册表模式，由 `PlatformEnvVarRegistry` 明确可管理 Key、用途说明、敏感标记和校验规则，避免后台变成任意键值中心。运行时统一通过 `PlatformEnvVarResolver` 取值，优先级为：
...
这些配置在 `.env.example`、`.env.server.example` 与 `application.yml` 中的初始值保持为空，示例文件只承担用途说明和本地启动回退入口。未纳入后台管理的数据库、Redis、MinIO、内部服务令牌、GitNexus 端口、Woodpecker provider、Hindsight 自身 LLM / Embedding / Reranker provider 等仍按部署配置文件管理，后续如果需要运行时切换再单独纳入注册表。
...
- `postgres`、`redis`、`minio`、`hindsight`、`hermes`、`woodpecker-server`、`woodpecker-agent` 走 Docker；显式设置 `WOODPECKER_ENABLED=false` 时跳过 Woodpecker
...
其中 Gitee 的全局 `enterpriseId` / `accessToken` 以及 PR 评审统计的 OA 用户 ID / OA 令牌，已不再要求在业务页面重复录入，而是统一走“系统设置-环境变量管理”：
...
- `docs/observability-technical-design-v1.md`：可观测性中心（入驻项目日志管理 + 系统健康度监控）技术设计
- `docs/public-saas-frontend-technical-design-v1.md`：公众 SaaS 产品前端技术设计
- `docs/public-credit-technical-design-v1.md`：公众端积分扣费技术设计（自动合并 AI 审核接入方案）
