...
| `refresh_models` | 用户头像弹窗内刷新按钮 | 强制 sidecar 联网重拉平台模型清单并重解析当前选中模型，让管理端新配置（visionRouting、输入模态等）无需重启 sidecar 即可生效 |
...
对话中的“查看完整计划”不再使用 `ContentDrawer`，而是自动展开右侧工具窗口并新增一个计划 Tab。右侧栏是通用容器：执行过程与计划均是可关闭的 Tab；右上角 `+` 菜单当前可重新打开执行过程，后续在同一入口增加终端等工具。每个来源会话只保留最新一份计划 Tab，保存标题和打开时的完整 Markdown 快照，并在栏内独立滚动和复制。Tab 状态仅保存在渲染层 `localStorage`，会话列表刷新后会清理来源不存在的计划。
