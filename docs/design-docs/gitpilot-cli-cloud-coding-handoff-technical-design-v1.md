...
7. 生产 Sandbox 采用 Docker、Kubernetes Job、Firecracker 还是现有基础设施可提供的其他隔离执行环境？在该决策完成前不得开放公众 SaaS。
...
11. `gitpilot-agent-core` 采用仓库内 workspace 依赖还是发布到私有 npm registry？需要结合 Docker build context 和 CLI 发布流水线确定，协议和包版本必须可独立追踪。
