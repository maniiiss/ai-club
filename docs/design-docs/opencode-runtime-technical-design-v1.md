...
- 非交互执行：`opencode run "<prompt>"`（不带子命令默认启动 TUI，不要混用）。
...
默认作为位置参数传入。opencode 常作为原生二进制分发，位置参数在多数平台可行；若 Windows 下 npm 包装层丢失多行参数（与 Claude `.ps1` 包装层同类问题），则降级为通过 stdin 传入。实现时按实际安装形态验证并固化。
...
```
opencode run "<prompt>" --dir <repo_dir> --format default --agent <plan|build> [--auto] [--model <model>]
```
...
- **prompt 传递兼容性**：Windows 下 npm 包装层可能丢多行参数，需按实际安装形态验证；保留 stdin 降级方案。
