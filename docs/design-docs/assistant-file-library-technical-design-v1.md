# Assistant 个人文件库与 Slash Skill 唤起技术设计 v1

## 背景

Assistant 原先会按自然语言关键词自动命中平台业务 Skill。这样普通聊天里提到“创建需求”“仓库扫描”等词时，也可能注入业务工具规则，导致个人协作问答和平台业务代理边界不清。本设计把平台业务 Skill 收口到显式 Slash 命令，并新增个人文件库作为每个用户自己的长期知识来源。

## 范围

- 只新增个人文件库，不新增项目共享文件库。
- 项目级知识继续由 Wiki 与 Qdrant 召回承担。
- 文件库默认参与 Assistant 普通问答召回，用户可按文件启停。
- 平台业务回答规则只通过结构化 `slashCommand` 启用，后端不从问题文本自动推断。

## 数据模型

新增 `assistant_file_library_item`：
...
资产绑定类型新增 `ASSISTANT_FILE_LIBRARY`，转换场景新增 `ASSISTANT_FILE_LIBRARY`。

## 后端链路

`AssistantFileLibraryController` 提供：
...
1. `DocumentAssetService.uploadAsset(file, "assistant-file-library")` 保存原始文件。
2. `DocumentMarkdownService.convert(assetId, ASSISTANT_FILE_LIBRARY, null)` 转 Markdown。
3. 保存 `assistant_file_library_item` 并绑定资产为 `ASSISTANT_FILE_LIBRARY`。
4. 使用 `WikiChunkingService` 切块，调用 `ModelConfigService` 生成 embedding。
5. 写入 Qdrant collection `assistant_file_library_chunks`，payload 携带 `ownerUserId`、`itemId`、`enabled`、标题、文件名和 chunk 文本。

问答前，`AssistantChatService` 先从 Hindsight 召回用户会话记忆，再从 Qdrant 按 `ownerUserId + enabled=true` 过滤召回个人文件库证据，最后按 Wiki 场景从 Qdrant 召回项目或空间知识证据，并交给 `AssistantPromptBuilder`。个人文件库召回失败只记日志并降级为空，不阻断主问答。

## Slash Skill 协议

请求体增加 `slashCommand`，覆盖 JSON 和 multipart：

- `AssistantSessionChatRequest.slashCommand`
- `AssistantChatRequest.slashCommand`
- `AssistantMultipartChatCommand.slashCommand`
...
`AssistantSkillContext` 暴露 `slashCommand()` 和 `hasSlashCommand(command)`。业务 Skill 的 `matches` 只判断结构化命令。无命令时，系统提示词仅包含基础协作规则。
