...
public class MemoryFactGraphService {
    ...
    public MemoryFactGraphSummary getProjectGraph(Long projectId) {
        ...
        List<String> graphBanks = resolveGraphBanks(projectId);
        ...
        if (graphs.isEmpty() && graphBanks.isEmpty()) {
            warnings.add("当前项目尚未解析到可用的记忆 bank。");
        }
        if (hindsightProperties.hasMemoryFactSharedBankId()) {
            warnings.add("共享 bank 当前仅参与事实召回，不参与实体图骨架聚合。");
        }
        return assembler.toGraphSummary(projectId, graphBanks, graphs, warnings);
    }
    ...
    public MemoryFactFactsResponse getFacts(Long projectId,
        ...
        MemoryFactGraphSummary graph = getProjectGraph(projectId);
        ...
        for (String bankId : resolveRecallBanks(projectId)) {
            try {
                ...
                List<HindsightClientService.MemoryWorldFact> fallbackFacts = tryFallbackFacts(projectId, bankId, entityId, edgeId, resolvedQuery, effectiveLimit, warnings, exception);
        ...
        return assembler.toFactsResponse(projectId, scopeType, scopeId, resolvedQuery, facts, warnings);
    }
    ...
    public MemoryFactEntityDetail getEntityDetail(Long projectId, String entityId) {
        ...
        MemoryFactFactsResponse factsResponse = getFacts(projectId, entityId, null, null, FACT_LIMIT);
        warnings.addAll(factsResponse.warnings());
        return assembler.toEntityDetail(projectId, entityId, node, detail, factsResponse.facts().stream()
     ...
     * 旧知识图谱依赖的 Wiki 投影方法只读复用，不在这里修改它的行为。
     */
    private List<String> resolveGraphBanks(Long projectId) {
     ...
     */
    private List<String> resolveRecallBanks(Long projectId) {
        LinkedHashSet<String> bankIds = new LinkedHashSet<>(resolveGraphBanks(projectId));
        if (hindsightProperties.hasMemoryFactSharedBankId()) {
    ...
    private List<HindsightClientService.MemoryWorldFact> tryFallbackFacts(Long projectId,
                                                                          String bankId,
                                                                          String entityId,
                                                                          String edgeId,
        ...
        try {
            ...
            if (!defaultString(entityId).isBlank()) {
                ...
                facts = hindsightMemoryFallbackService.searchFacts(resolveGraphBanks(projectId), resolvedQuery, limit);
            }
            if (!facts.isEmpty()) {
