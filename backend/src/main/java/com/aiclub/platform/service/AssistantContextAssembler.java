...
/**
 * 按当前页面上下文组装 Assistant 所需的业务摘要，避免直接把数据库结构暴露给大模型。
 */
@Service
public class AssistantContextAssembler {
    ...
    public AssistantContextAssembler(PlatformStoreService platformStoreService,
     ...
     */
    public AssistantConversationContext assemble(AssistantChatRequest request, CurrentUserInfo currentUser) {
    ...
    /**
     * 首页场景聚合我的任务、未读通知和近期异常，方便 Assistant 给出工作优先级建议。
     */
    private AssistantConversationContext buildDashboardContext(CurrentUserInfo currentUser) {
        int limit = assistantProperties.getMaxContextMessages();
        ...
        references.add(new AssistantReferenceSummary("DASHBOARD", null, "首页看板", "/dashboard"));
        ...
        return new AssistantConversationContext(
                "dashboard",
                null,
                null,
                null,
    ...
    /**
     * 项目场景聚合项目摘要、近期迭代和近期工作项，方便 Assistant 从项目管理视角作答。
     */
    private AssistantConversationContext buildProjectContext(String routeName, Long projectId, CurrentUserInfo currentUser) {
        ProjectSummary project = platformStoreService.getProject(projectId);
        PageResponse<TaskSummary> taskPage = platformStoreService.pageTasks(1, assistantProperties.getMaxContextMessages(), null, null, null, projectId, null);
    ...
    /**
     * 任务场景聚合任务详情、最近评论和最近 Agent 执行结果，帮助 Assistant 从执行上下文作答。
     */
    private AssistantConversationContext buildTaskContext(String routeName, Long taskId, CurrentUserInfo currentUser) {
        ...
        List<TaskAgentRunSummary> recentRuns = taskAgentRunService.listRecentRuns(taskId).stream()
                .limit(assistantProperties.getMaxContextMessages())
                .toList();
        List<AssistantReferenceSummary> references = List.of(
                taskReference(task),
     ...
     * 这样 Assistant 在回答发版内容、缺陷修复数量和需求清单时，可以直接基于当前迭代上下文作答。
     */
    private AssistantConversationContext buildIterationContext(String routeName,
                                                            Long projectId,
                                                            Long iterationId,
     ...
     */
    private AssistantConversationContext buildGlobalContext(String routeName, CurrentUserInfo currentUser) {
        StringBuilder context = new StringBuilder();
        context.append("## 当前场景\n")
                .append("全局助手\n\n")
                .append("## 当前用户\n")
    ...
    /**
     * Wiki 页面场景聚合当前空间、当前页面和相关页面，方便 Assistant 基于空间知识作答。
     */
    private AssistantConversationContext buildWikiPageContext(String routeName, Long spaceId, Long wikiPageId, CurrentUserInfo currentUser) {
    ...
    /**
     * Wiki 空间场景聚合空间摘要和最近页面，方便 Assistant 从知识空间视角作答。
     */
    private AssistantConversationContext buildWikiSpaceContext(String routeName, Long spaceId, CurrentUserInfo currentUser) {
        WikiSpaceDetail space = wikiSpaceService.getSpaceDetail(spaceId);
    ...
    private void appendTaskList(StringBuilder builder, List<TaskSummary> tasks) {
        ...
        for (TaskSummary task : tasks.stream().limit(assistantProperties.getMaxContextMessages()).toList()) {
            builder.append("- ")
    ...
    /**
     * 迭代摘要里保留编号、类型和状态，便于 Assistant 直接生成发版说明而不必再次搜索对象。
     */
    ...
    private void appendNotifications(StringBuilder builder, List<NotificationItem> notifications) {
        ...
        for (NotificationItem item : notifications.stream().limit(assistantProperties.getMaxContextMessages()).toList()) {
            builder.append("- ")
    ...
    private void appendWikiPages(StringBuilder builder, List<WikiSpacePageSummary> pages) {
        ...
        for (WikiSpacePageSummary page : pages.stream().limit(assistantProperties.getMaxContextMessages()).toList()) {
            builder.append("- ")
    ...
    private List<TaskSummary> trimTaskList(List<TaskSummary> tasks) {
        ...
        return tasks.stream().limit(assistantProperties.getMaxContextMessages()).toList();
    }
    ...
    private AssistantReferenceSummary projectReference(ProjectSummary project) {
    ...
    private AssistantReferenceSummary taskReference(TaskSummary task) {
        return new AssistantReferenceSummary(
                "TASK",
                task.id(),
    ...
    private AssistantReferenceSummary iterationReference(IterationSummary iteration) {
        return new AssistantReferenceSummary(
                "ITERATION",
                iteration.id(),
    ...
    /**
     * Assistant 需要的最小上下文载体，只保留问答真正需要的摘要信息。
     */
    public record AssistantConversationContext(
         ...
         * 兼容旧测试构造方式。
         */
        public AssistantConversationContext(String sceneCode,
                                         Long projectId,
                                         Long taskId,
                                         String roleName,
                                         List<AssistantReferenceSummary> references,
