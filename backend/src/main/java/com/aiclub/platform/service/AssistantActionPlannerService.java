...
/**
 * Assistant 写操作动作规划服务。
 * Assistant 通过 write tool call 表达意图，平台负责把它们安全映射成待确认动作卡片。
 */
@Service
public class AssistantActionPlannerService {
    ...
    /**
     * 将 Assistant 发起的写工具调用转换为前端可执行的确认卡片。
     */
    public AssistantActionSummary createActionFromToolCall(String toolCode,
    ...
    /**
     * 生成仓库规范扫描任务创建动作卡片。
     * 这里优先复用当前会话中已绑定的仓库对象，避免 Assistant 为了补齐 bindingId 再要求用户输入内部 ID。
     */
    private AssistantActionSummary createRepositoryScanAction(Map<String, Object> arguments,
                                                           AssistantGroundingState groundingState) {
     ...
     * 生成执行中心任务创建动作卡片。
     */
    private AssistantActionSummary createExecutionAction(Map<String, Object> arguments,
                                                      AssistantGroundingState groundingState,
                                                      String userQuestion) {
     ...
     * 生成工作项草稿创建动作卡片。
     */
    private AssistantActionSummary createWorkItemAction(Map<String, Object> arguments,
                                                     AssistantGroundingState groundingState) {
        Long projectId = resolveLong(arguments.get("projectId"));
        if (projectId == null && groundingState != null && groundingState.boundSlot("project") != null) {
            projectId = groundingState.boundSlot("project").entityId();
     ...
     * 生成测试计划草稿创建动作卡片。
     */
    private AssistantActionSummary createTestPlanAction(Map<String, Object> arguments,
                                                     AssistantGroundingState groundingState) {
        Long projectId = resolveLong(arguments.get("projectId"));
        if (projectId == null && groundingState != null && groundingState.boundSlot("project") != null) {
            projectId = groundingState.boundSlot("project").entityId();
        ...
        params.put("projectId", projectId);
        params.put("iterationId", iterationId);
        params.put("name", defaultString(arguments.get("name"), "GitPilot 测试计划草稿"));
        params.put("description", defaultString(arguments.get("description"), "由 GitPilot 根据当前上下文创建的测试计划草稿。"));
        return new AssistantActionSummary(
    ...
    private String buildTitle(String content, String workItemType) {
        ...
        if (normalized.isBlank()) {
            return "GitPilot 创建的" + workItemType + "草稿";
        }
