...
public class GitPilotCliService {
    ...
    public List<CliModelSummary> listModels() {
        return modelConfigService.listEnabledOptions(ModelConfigService.MODEL_TYPE_CHAT).stream()
                ...
                .map(item -> new CliModelSummary(item.id(), item.name(), item.provider(), item.modelName(), item.description(), item.openaiApiMode(), item.contextLength(), item.maxOutputTokens(), item.inputModalities()))
                .toList();
    }
