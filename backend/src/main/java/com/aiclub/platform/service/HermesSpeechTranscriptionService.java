 ...
 */
@Service
public class HermesSpeechTranscriptionService {

    private static final Logger log = LoggerFactory.getLogger(HermesSpeechTranscriptionService.class);
    ...
    private final HermesSpeechProperties hermesSpeechProperties;
    private final ObjectMapper objectMapper;
    private final RestClient restClient;

    public HermesSpeechTranscriptionService(HermesSpeechProperties hermesSpeechProperties,
                                            ObjectMapper objectMapper) {
        this.hermesSpeechProperties = hermesSpeechProperties;
        this.objectMapper = objectMapper;
        SimpleClientHttpRequestFactory requestFactory = new SimpleClientHttpRequestFactory();
        requestFactory.setConnectTimeout((int) Duration.ofSeconds(10).toMillis());
        requestFactory.setReadTimeout((int) Duration.ofSeconds(hermesSpeechProperties.getTimeoutSeconds()).toMillis());
    ...
    /**
     * 把前端录制的短音频转写成文本。
     * 业务意图是把语音输入收口到 Hermes 现有 question 文本链路，避免改动聊天协议。
     */
    public String transcribe(MultipartFile file) {
        ...
        try {
            ...
            log.warn("Hermes speech transcription returned empty text: fileName={}, contentType={}, size={}, autoResponse={}, zhResponse={}",
                    audioFile.fileName(),
            ...
            throw new IllegalArgumentException("Hermes 语音转写失败：服务未返回有效文本，请尽量靠近麦克风并完整说一句话");
        } catch (IllegalArgumentException exception) {
            throw exception;
        } catch (HttpStatusCodeException exception) {
            throw new IllegalArgumentException("Hermes 语音转写失败：" + extractErrorMessage(exception.getResponseBodyAsString()));
        } catch (ResourceAccessException exception) {
            throw new IllegalArgumentException("Hermes 语音转写失败，请稍后重试");
    ...
    private void validateConfiguration() {
        if (!hasText(hermesSpeechProperties.getApiKey())) {
            throw new IllegalArgumentException("请先配置 Hermes 语音转写 API Key");
        }
        if (!hasText(hermesSpeechProperties.getBaseUrl())) {
            throw new IllegalArgumentException("请先配置 Hermes 语音转写服务地址");
        }
        if (!hasText(hermesSpeechProperties.getModel())) {
            throw new IllegalArgumentException("请先配置 Hermes 语音转写模型");
        }
    }
    ...
    private ValidatedAudioFile validateFile(MultipartFile file) {
        ...
        if (file.getSize() > hermesSpeechProperties.getMaxFileSize().toBytes()) {
            throw new IllegalArgumentException("语音文件大小不能超过" + hermesSpeechProperties.getMaxFileSize().toMegabytes() + "MB");
        }
    ...
    private String requestTranscription(ValidatedAudioFile audioFile, String language) {
        String responseBody = restClient.post()
                .uri(hermesSpeechProperties.getBaseUrl() + "/audio/transcriptions")
                .headers(headers -> headers.setBearerAuth(hermesSpeechProperties.getApiKey()))
    ...
    private MultiValueMap<String, Object> buildMultipartBody(ValidatedAudioFile audioFile, String language) {
        ...
        formData.add("model", hermesSpeechProperties.getModel());
    ...
    private String extractTextOrNull(String body) {
        try {
            ...
            throw new IllegalArgumentException("Hermes 语音转写失败：响应格式不正确");
        }
    }
    ...
    private void logWavDiagnostics(ValidatedAudioFile audioFile) {
        ...
        if (bytes.length < 44) {
            log.warn("Hermes speech wav diagnostics: file too small, size={}", bytes.length);
            return;
        }
        ...
        log.warn("Hermes speech wav diagnostics: channels={}, sampleRate={}, bitsPerSample={}, durationSeconds={}, normalizedPeak={}, normalizedMeanAbs={}",
                channelCount,
                sampleRate,
                bitsPerSample,
