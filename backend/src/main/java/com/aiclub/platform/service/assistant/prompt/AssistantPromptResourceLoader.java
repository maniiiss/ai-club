...
/**
 * 统一加载 Assistant Prompt 相关 Markdown 资源。
 * 资源文件缺失时直接抛错，避免生产环境静默退化回旧的硬编码提示词。
 */
@Component
public class AssistantPromptResourceLoader {
    ...
    public String readRequiredMarkdown(String classpathLocation) {
        if (classpathLocation == null || classpathLocation.isBlank()) {
            throw new IllegalArgumentException("Assistant Prompt 资源路径不能为空");
        }
    ...
    private String loadMarkdown(String classpathLocation) {
        ...
        if (!resource.exists()) {
            throw new IllegalStateException("Assistant Prompt 资源不存在: " + classpathLocation);
        }
        try {
            return StreamUtils.copyToString(resource.getInputStream(), StandardCharsets.UTF_8).trim();
        } catch (IOException exception) {
            throw new IllegalStateException("读取 Assistant Prompt 资源失败: " + classpathLocation, exception);
        }
    }
}
