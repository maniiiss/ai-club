...
public class AssistantProperties {
    ...
    public AssistantProperties(String baseUrl,
                            String apiKey,
                            String model,
                            int timeoutSeconds,
    ...
    private AssistantProperties(String baseUrl,
                             String apiKey,
                             String model,
    ...
    public String getBaseUrl() {
        // Assistant 网关地址改回部署配置维护，避免后台运行时覆盖与容器接入参数漂移。
        return trimTrailingSlash(baseUrl);
    }

    public String getApiKey() {
        // Assistant 网关鉴权密钥同样只认部署配置，后台环境变量管理不再覆盖这里。
