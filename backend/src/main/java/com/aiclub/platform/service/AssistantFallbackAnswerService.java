...
/**
 * Assistant tool loop 被平台中断时的本地总结器。
 * 这里只处理歧义选择、写操作确认和工具循环失败三类场景。
 */
@Service
public class AssistantFallbackAnswerService {

    /**
     * 多候选歧义场景下，明确要求用户先完成对象选择。
     */
    public String composeSelectionSummary(List<AssistantSelectionCard> selectionCards) {
     ...
     */
    public String composeActionSummary(List<AssistantActionSummary> actions) {
        StringBuilder builder = new StringBuilder("我已经根据当前上下文准备好了可执行动作，但出于安全考虑需要你先确认。\n\n");
        for (AssistantActionSummary action : actions) {
            builder.append("- ")
