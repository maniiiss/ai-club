...
/**
 * 验证 Assistant 个人文件库的归属隔离、文档转换与 Qdrant 向量索引行为。
 */
@ExtendWith(MockitoExtension.class)
class AssistantFileLibraryServiceTests {
    ...
    @Mock
    private AssistantFileLibraryItemRepository assistantFileLibraryItemRepository;
    ...
    void shouldUploadConvertBindAndIndexPersonalFile() {
        AssistantFileLibraryService service = createService();
        ...
        when(assistantFileLibraryItemRepository.save(any(AssistantFileLibraryItemEntity.class)))
                .thenAnswer(invocation -> {
                    AssistantFileLibraryItemEntity item = invocation.getArgument(0);
                    if (item.getId() == null) {
                        item.setId(77L);
                    }
    ...
    void shouldListOnlyCurrentUserItems() {
        ...
        CurrentUserInfo currentUser = currentUser();
        when(authService.currentUser()).thenReturn(currentUser);
        when(assistantFileLibraryItemRepository.findAllByOwnerUser_IdAndTitleContainingIgnoreCaseOrderByUpdatedAtDescIdDesc(5L, "需求"))
                .thenReturn(List.of(fileItem(77L, ownerUser(5L), true, "INDEXED")));

        List<AssistantFileLibraryItemSummary> summaries = service.list("需求");

        assertThat(summaries).hasSize(1);
        assertThat(summaries.get(0).id()).isEqualTo(77L);
        verify(assistantFileLibraryItemRepository).findAllByOwnerUser_IdAndTitleContainingIgnoreCaseOrderByUpdatedAtDescIdDesc(5L, "需求");
    }
    ...
    void shouldDeleteOwnedItemAndQdrantVectors() {
        ...
        CurrentUserInfo currentUser = currentUser();
        when(authService.currentUser()).thenReturn(currentUser);
        when(assistantFileLibraryItemRepository.findByIdAndOwnerUser_Id(77L, 5L))
    ...
    void shouldSkipDisabledItemsWhenBuildingEvidence() {
        AssistantFileLibraryService service = createService();
        when(assistantFileLibraryItemRepository.findAllByOwnerUser_IdAndEnabledTrueOrderByUpdatedAtDescIdDesc(5L))
                .thenReturn(List.of());
    ...
    void shouldReportVectorSearchMissInsteadOfUsingMarkdownFallback() {
        AssistantFileLibraryService service = createService();
        AssistantFileLibraryItemEntity item = fileItem(77L, ownerUser(5L), true, "INDEXED");
        item.setTitle("2025年个人总结");
        item.setMarkdown("# 2025年个人总结\n\n年度重点包括项目交付、知识库建设和测试自动化推进。");
        when(assistantFileLibraryItemRepository.findAllByOwnerUser_IdAndEnabledTrueOrderByUpdatedAtDescIdDesc(5L))
        ...
        String markdown = service.buildEvidenceMarkdown(currentUser(), "我的个人资料里写了什么");

        assertThat(markdown).isBlank();
        assertThat(markdown).doesNotContain("项目交付");
        assertThat(markdown).doesNotContain("知识库建设");
    }
    ...
    void shouldFallbackToIndexedFileTitleWhenVectorSearchMissesShortNameQuery() {
        AssistantFileLibraryService service = createService();
        AssistantFileLibraryItemEntity item = fileItem(77L, ownerUser(5L), true, "INDEXED");
        item.setTitle("某候选人简历");
        item.getDocumentAsset().setFileName("candidate-resume.docx");
        item.setMarkdown("# 某候选人简历\n\n某候选人曾负责项目交付、智能助手建设和自动化测试推进。");
        when(assistantFileLibraryItemRepository.findAllByOwnerUser_IdAndEnabledTrueOrderByUpdatedAtDescIdDesc(5L))
                .thenReturn(List.of(item));
        ...
        String markdown = service.buildEvidenceMarkdown(currentUser(), "某候选人是谁");

        assertThat(markdown).contains("个人文件库证据");
        assertThat(markdown).contains("某候选人简历");
        assertThat(markdown).contains("项目交付");
    ...
    void shouldReportIndexFailureWhenPersonalDocumentVectorIndexIsNotReady() {
        ...
        item.setMarkdown("# 2025年述职报告\n\n工作成果包括示例项目交付、个人知识库文档上传和自动化测试建设。");
        item.setLastError("Assistant 文件库向量索引未启用或未配置 Embedding 模型");
        when(assistantFileLibraryItemRepository.findAllByOwnerUser_IdAndEnabledTrueOrderByUpdatedAtDescIdDesc(5L))
                .thenReturn(List.of(item));
    ...
    void shouldIgnoreVectorHitsFromItemsThatAreNotIndexed() {
        AssistantFileLibraryService service = createService();
        AssistantFileLibraryItemEntity indexedItem = fileItem(77L, ownerUser(5L), true, "INDEXED");
        AssistantFileLibraryItemEntity failedItem = fileItem(88L, ownerUser(5L), true, "FAILED");
    ...
    void shouldRecordReindexFailure() {
        ...
        AssistantFileLibraryItemSummary summary = service.reindex(77L);

        assertThat(summary.indexStatus()).isEqualTo("FAILED");
        assertThat(summary.lastError()).contains("转换服务不可用");
    }

    private AssistantFileLibraryService createService() {
        return new AssistantFileLibraryService(
                authService,
                documentAssetService,
                documentMarkdownService,
                assistantFileLibraryItemRepository,
    ...
    private AssistantFileLibraryItemEntity fileItem(Long id, UserEntity owner, boolean enabled, String indexStatus) {
