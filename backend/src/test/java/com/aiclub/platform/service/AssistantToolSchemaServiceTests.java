...
/**
 * 验证 Assistant 工具 schema 暴露符合仓库扫描闭环的接入约束。
 */
@ExtendWith(MockitoExtension.class)
class AssistantToolSchemaServiceTests {

    @Mock
    private PlatformToolRegistry platformToolRegistry;

    /**
     * 只读工具目录应包含扫描规则集列表，供 Assistant 在发起扫描前先完成规则集确认。
     */
    @Test
    void shouldExposeRepositoryScanRulesetToolToReadableToolList() {
        AssistantToolSchemaService service = new AssistantToolSchemaService(platformToolRegistry, new ObjectMapper());
        ...
        List<AssistantCallableTool> tools = service.listCallableTools(currentUser());

        assertThat(tools).extracting(AssistantCallableTool::toolCode)
    ...
    void shouldExposeRepositoryScanStartToolWithRequiredRulesetCode() {
        ...
        List<AssistantCallableTool> tools = service.listCallableWriteTools(currentUser());
