...
/**
 * 验证 Assistant 上下文装配在首页、任务和无权限回退场景下都能输出稳定结果。
 */
@SpringBootTest
@Transactional
class AssistantContextAssemblerTests {
    ...
    void shouldBuildDashboardContext() {
        ...
        AssistantContextAssembler.AssistantConversationContext context = assistantContextAssembler.assemble(
                new AssistantChatRequest("我今天最该推进什么", "dashboard", null, null, null, null, null, null, null),
    ...
    /**
     * 任务场景应输出任务详情、项目信息和评论摘要，供 Assistant 进行任务级回答。
     */
    @Test
    void shouldBuildTaskContextWithTaskDetails() {
        ...
        AssistantContextAssembler.AssistantConversationContext context = assistantContextAssembler.assemble(
                new AssistantChatRequest("这个任务为什么延期了", "project-iterations", project.getId(), task.id(), null, null, null, null, null),
    ...
    /**
     * 迭代详情页应优先装配当前迭代摘要，便于 Assistant 直接生成发版内容、需求和缺陷统计。
     */
    @Test
    void shouldBuildIterationContextWithReleaseSummary() {
        ...
        AssistantContextAssembler.AssistantConversationContext context = assistantContextAssembler.assemble(
                new AssistantChatRequest("帮我总结这个迭代的发版内容", "project-iterations", project.getId(), null, iteration.id(), null, null, null, null),
                toCurrentUserInfo(creator)
        );
    ...
    void shouldFallbackToGlobalContextWhenTaskIsNotVisible() {
        ...
        loginAs(outsider);
        AssistantContextAssembler.AssistantConversationContext context = assistantContextAssembler.assemble(
                new AssistantChatRequest("这个任务现在怎么样", "project-iterations", project.getId(), task.id(), null, null, null, null, null),
