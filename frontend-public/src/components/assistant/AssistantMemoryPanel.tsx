...
import type { AssistantFileLibraryItem, AssistantMemoryFactItem, AssistantUserMemoryItem } from '@/src/types/assistant'
...
export const AssistantMemoryPanel = ({ onClose }: AssistantMemoryPanelProps) => {
  ...
  const [fileItems, setFileItems] = useState<AssistantFileLibraryItem[]>([])
  ...
  const [fileDeleteTarget, setFileDeleteTarget] = useState<AssistantFileLibraryItem | null>(null)
  ...
  const loadFiles = async (nextQuery = fileQuery) => {
    ...
    setError('')
    try {
      setFileItems(await listAssistantFileLibraryItems(nextQuery))
    } catch (err) {
      setError(getErrorMessage(err))
    } finally {
      setLoading(false)
  ...
  const confirmDeleteFile = async () => {
    ...
    setError('')
    try {
      await deleteAssistantFileLibraryItem(fileDeleteTarget.id)
