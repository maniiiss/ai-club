...
interface AssistantActionCardsProps {
  ...
  disabled: boolean
  onConfirm: (action: AssistantActionItem, index: number, actionKey: string) => void
}

export const AssistantActionCards = ({
  actions,
  executedActionKeys,
  executingActionKey,
  disabled,
    ...
    <section className="rounded-lg border border-amber-200 bg-amber-50/70 p-3">
      ...
      <div className="space-y-2">
        {actions.map((action, index) => {
          const key = computeAssistantActionKey(action, index)
