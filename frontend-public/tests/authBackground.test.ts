...
describe('GitPilot authentication background', () => {
  it('mounts the animated Git branch background in the auth layout', () => {
    ...
    assert.match(styles, /var\(--auth-dark-base\)/)
    assert.match(layout, /auth-layout-root/)
    assert.match(styles, /var\(--auth-route-blue\)/)
    assert.match(background, /var\(--auth-route-teal\)/)
  })
})
