...
export interface RightPanelTabsState {
	...
	reviewOpen: boolean;
	/** Git（受限 sidecar Git 操作面板）是可关闭的工具页签，可由右侧 + 菜单再次打开。 */
	gitOpen: boolean;
...
export function normalizeRightPanelTabs(value: Partial<RightPanelTabsState> | null | undefined): RightPanelTabsState {
	...
	const gitOpen = value?.gitOpen === true;
	...
	const activeTabId = requestedActive === 'execution' && executionOpen
		...
		|| requestedActive === 'git' && gitOpen
		|| requestedActive && plans.some((tab) => tab.id === requestedActive)
		? requestedActive
		: executionOpen ? 'execution' : reviewOpen ? 'review' : filesOpen ? 'files' : gitOpen ? 'git' : plans[0]?.id ?? null;
	return { plans, executionOpen, filesOpen, reviewOpen, gitOpen, activeTabId };
}

function loadRightPanelTabs(): RightPanelTabsState {
	try {
		const stored = localStorage.getItem(RIGHT_PANEL_TABS_KEY);
		return stored ? normalizeRightPanelTabs(JSON.parse(stored) as Partial<RightPanelTabsState>) : { plans: [], executionOpen: true, filesOpen: false, reviewOpen: false, gitOpen: false, activeTabId: 'execution' };
	} catch {
		return { plans: [], executionOpen: true, filesOpen: false, reviewOpen: false, gitOpen: false, activeTabId: 'execution' };
	}
}
...
interface WorkbenchStore {
	...
	/** 打开右侧 Git 页签（受限 sidecar Git 面板），同时展开右侧栏。 */
	openGitPanelTab: () => void;
...
export const useWorkbenchStore = create<WorkbenchStore>()((set, get) => ({
	...
	openGitPanelTab: () => {
		...
		const layout = normalizeLayoutPreferences({ ...get().layout, rightCollapsed: false });
		saveLayout(layout);
		const rightPanelTabs = { ...state, gitOpen: true, activeTabId: 'git' };
		saveRightPanelTabs(rightPanelTabs);
		set({ rightPanelTabs, layout });
	},
	activateRightPanelTab: (tabId) => {
		...
		if (state.activeTabId === tabId
			...
			|| tabId === 'git' && !state.gitOpen
			|| tabId !== 'execution' && tabId !== 'files' && tabId !== 'review' && tabId !== 'git' && !state.plans.some((tab) => tab.id === tabId)) return;
	...
	closeRightPanelTab: (tabId) => {
		const state = get().rightPanelTabs;
		if (tabId === 'execution') {
			const activeTabId = state.activeTabId === 'execution' ? state.reviewOpen ? 'review' : state.filesOpen ? 'files' : state.gitOpen ? 'git' : state.plans[0]?.id ?? null : state.activeTabId;
		...
		if (tabId === 'review') {
			const activeTabId = state.activeTabId === 'review' ? state.filesOpen ? 'files' : state.gitOpen ? 'git' : state.plans[0]?.id ?? (state.executionOpen ? 'execution' : null) : state.activeTabId;
		...
		if (tabId === 'files') {
			const activeTabId = state.activeTabId === 'files' ? state.plans[0]?.id ?? (state.reviewOpen ? 'review' : state.gitOpen ? 'git' : state.executionOpen ? 'execution' : null) : state.activeTabId;
		...
		if (tabId === 'git') {
			const activeTabId = state.activeTabId === 'git' ? state.filesOpen ? 'files' : state.reviewOpen ? 'review' : state.plans[0]?.id ?? (state.executionOpen ? 'execution' : null) : state.activeTabId;
			const rightPanelTabs = { ...state, gitOpen: false, activeTabId };
			saveRightPanelTabs(rightPanelTabs);
			set({ rightPanelTabs });
			return;
		}
		...
		const activeTabId = state.activeTabId === tabId
			? plans[index]?.id ?? plans[index - 1]?.id ?? (state.executionOpen ? 'execution' : state.reviewOpen ? 'review' : state.filesOpen ? 'files' : state.gitOpen ? 'git' : null)
			: state.activeTabId;
	...
	reconcileRightPanelTabs: (sessionPaths) => {
		...
		const activeTabId = state.activeTabId === 'execution' && state.executionOpen
			...
			|| state.activeTabId === 'git' && state.gitOpen
			|| plans.some((tab) => tab.id === state.activeTabId)
			? state.activeTabId
			: state.executionOpen ? 'execution' : state.reviewOpen ? 'review' : state.filesOpen ? 'files' : state.gitOpen ? 'git' : plans[0]?.id ?? null;
