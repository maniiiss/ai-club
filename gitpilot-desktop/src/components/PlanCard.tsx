...
import { Check, ClipboardText, ListChecks, ArrowUpRight } from '@phosphor-icons/react';
	...
	return (
		<article className={styles.card} aria-label="实施计划">
			...
			<header className={styles.header}>
				<span className={styles.label}><ListChecks weight="regular" size={16} aria-hidden="true" />计划</span>
				<Hint content={copied ? '已复制' : '复制完整计划'}><Button type="button" variant="ghost" size="icon-sm" className={styles.copyButton} onClick={() => void copy()} aria-label={copied ? '已复制计划' : '复制计划'}>
					{copied ? <Check weight="bold" size={15} /> : <ClipboardText weight="regular" size={15} />}
				</Button></Hint>
			</header>
			<div className={styles.preview}>
				...
				<div className={styles.footer}>
					<Button type="button" variant="unstyled" size="sm" className={styles.openButton} onClick={open} disabled={!selectedSessionPath}>
						查看完整计划 <ArrowUpRight weight="regular" size={14} aria-hidden="true" />
					</Button>
				</div>
			</div>
		</article>
