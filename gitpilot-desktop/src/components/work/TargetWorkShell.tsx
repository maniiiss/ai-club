import { ArrowsIn, ArrowsOut, BracketsCurly, ChatCircleDots, CircleNotch, ClipboardText, File, FileArchive, FileCode, FileCsv, FileImage, FilePlus, FileText, Folder, FolderOpen, Network, PencilSimple, PaperPlaneTilt, Plus, Sparkle, Square, Trash, X } from '@phosphor-icons/react';
...
function FileTypeIcon({ file }: { file: WorkFile }) {
	...
	if (['ts', 'tsx', 'js', 'jsx', 'css', 'html', 'py', 'java', 'go', 'rs', 'sql', 'sh'].includes(extension)) return <FileCode weight="regular" aria-hidden="true" />;
	if (['json', 'jsonl'].includes(extension)) return <BracketsCurly weight="regular" aria-hidden="true" />;
	if (['png', 'jpg', 'jpeg', 'gif', 'svg', 'webp', 'ico'].includes(extension)) return <FileImage weight="regular" aria-hidden="true" />;
	if (['csv', 'xls', 'xlsx'].includes(extension)) return <FileCsv weight="regular" aria-hidden="true" />;
	if (['zip', 'tar', 'gz', '7z', 'rar'].includes(extension)) return <FileArchive weight="regular" aria-hidden="true" />;
	if (['md', 'txt', 'log'].includes(extension)) return <FileText weight="regular" aria-hidden="true" />;
	return <File weight="regular" aria-hidden="true" />;
}
...
function WorkTaskRow({ task, active, running, indent, onSelect }: { task: WorkTask; active: boolean; running: boolean; indent: boolean; onSelect: (id: string) => void }) {
	...
	return <Hint content={running ? `${label}（进行中）` : label}><Button type="button" variant="ghost" size="sm" data-sidebar-menu-kind="work-task" data-work-task-id={task.id} data-session-cwd={task.workspacePath} aria-busy={running || undefined} onClick={() => onSelect(task.id)} className={`${sidebarStyles.taskRow} ${active ? sidebarStyles.taskActive : ''}`} style={indent ? { paddingLeft: '31px' } : undefined}><span className={sidebarStyles.label}>{label}</span>{running && <CircleNotch weight="bold" className={sidebarStyles.taskLoading} aria-hidden="true" />}</Button></Hint>;
}
	...
	return <Collapsible open={expanded} onOpenChange={(open) => onExpandedChange(workspace.path, open)}>
		<div className={sidebarStyles.projectBlock} data-sidebar-menu-kind="work-project" data-project-path={workspace.path} data-workspace-path={workspace.path} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
			<div className={sidebarStyles.projectRow}>
				<CollapsibleTrigger asChild><Button type="button" variant="ghost" size="icon-sm" className={sidebarStyles.folderTrigger} aria-label={expanded ? `收起 ${workspace.name}` : `展开 ${workspace.name}`} title={expanded ? `收起 ${workspace.name}` : `展开 ${workspace.name}`}>{expanded ? <FolderOpen weight="regular" className={sidebarStyles.typeIcon} aria-hidden="true" /> : <Folder weight="regular" className={sidebarStyles.typeIcon} aria-hidden="true" />}</Button></CollapsibleTrigger>
				...
				<Hint content="在该工作空间新建任务"><Button type="button" variant="ghost" size="icon-sm" onClick={() => onCreate(workspace.path)} className={cn(sidebarStyles.action, hovered ? sidebarStyles.actionVisible : '')} aria-label={`在 ${workspace.name} 新建任务`}><FilePlus weight="regular" /></Button></Hint>
			</div>
...
function WorkTaskSidebar({ tasks, activeTaskId, runningTaskId, workspaces, currentWorkspacePath, onCreate, onSelect, onAddWorkspace, onSelectWorkspace }: WorkTaskSidebarProps) {
	...
	return <aside className={sidebarStyles.root} aria-label="Work 工作空间与任务">
		<div className={sidebarStyles.primaryNav} aria-label="会话操作"><Hint content="新建对话"><Button type="button" variant="ghost" size="sm" className={sidebarStyles.primaryNavItem} onClick={() => onCreate(null)} aria-label="新建对话"><ChatCircleDots weight="regular" aria-hidden="true" /><span>新对话</span></Button></Hint><ConversationHistorySearch items={tasks} getKey={(task) => task.id} getTitle={(task) => getWorkTaskTitle(task.title, task.messages.find((message) => message.role === 'user')?.text)} getSearchText={(task) => [getWorkTaskTitle(task.title, task.messages.find((message) => message.role === 'user')?.text), ...task.messages.map((message) => message.text)].join(' ')} getUpdatedAt={(task) => task.updatedAt} onSelect={(task) => onSelect(task.id)} label="搜索历史任务" triggerText="搜索" triggerVariant="nav" /></div>
		<header className={sidebarStyles.header}><div className={sidebarStyles.headerCopy}><span>工作空间</span></div><div className={sidebarStyles.headerActions}>{allWorkspacesCollapsed ? <Hint content="展开全部工作空间"><Button type="button" variant="ghost" size="icon-sm" className={sidebarStyles.headerAction} onClick={expandAllWorkspaces} disabled={workspaces.length === 0} aria-label="展开全部工作空间"><ArrowsOut weight="regular" aria-hidden="true" /></Button></Hint> : <Hint content="收起全部工作空间"><Button type="button" variant="ghost" size="icon-sm" className={sidebarStyles.headerAction} onClick={collapseAllWorkspaces} disabled={!hasExpandedWorkspace} aria-label="收起全部工作空间"><ArrowsIn weight="regular" aria-hidden="true" /></Button></Hint>}<Hint content="添加工作空间"><Button type="button" variant="ghost" size="icon-sm" className={sidebarStyles.headerAction} onClick={onAddWorkspace} aria-label="添加工作空间"><Plus weight="bold" aria-hidden="true" /></Button></Hint></div></header>
		<ScrollArea fitContent className={sidebarStyles.scroll}><div className={sidebarStyles.content}>
			...
			{workspaces.length > 0 && <div className={sidebarStyles.sectionHeader}><span>未分组任务</span><Hint content="新建任务"><Button type="button" variant="ghost" size="icon-sm" className={sidebarStyles.sectionAction} onClick={() => onCreate(null)} aria-label="新建任务"><FilePlus weight="regular" /></Button></Hint></div>}
...
function WorkInputBox({ disabled, running, onSend, onAbort, inline, pendingWorkItem, onClearPendingWorkItem }: { disabled: boolean; running: boolean; onSend: (text: string) => void; onAbort: () => void; inline?: boolean; pendingWorkItem: RpcWorkItemDetail | null; onClearPendingWorkItem: () => void }) {
	...
	return <div className={rootClassName}><div className={inputStyles.surface}>{showPalette && <CommandPalette commands={commands} query={text.slice(1)} onPick={pickCommand} onDismiss={() => setShowPalette(false)} />}{pendingWorkItem ? <div className={styles.collabPendingChip}><ClipboardText weight="regular" size={12} aria-hidden="true" /><span className={styles.collabPendingName}>{pendingWorkItem.workItemCode} {pendingWorkItem.name}</span><button type="button" onClick={onClearPendingWorkItem} aria-label="移除待发送工作项"><X weight="bold" size={12} /></button></div> : null}<div className={inputStyles.composerRow}><textarea className={`${inputStyles.editorShell} ${inputStyles.editorSurface} ${styles.workEditor}`} value={text} onChange={(event) => onTextChange(event.target.value)} onKeyDown={onKeyDown} disabled={disabled} placeholder="描述你要推进的工作" aria-label="发送 Work 消息" /></div><div className={inputStyles.toolbar}><div className={inputStyles.actions}><ModelPicker />{running ? <Hint content="停止"><Button type="button" variant="ghost" size="icon" onClick={onAbort} aria-label="停止"><Square weight="bold" size={15} /></Button></Hint> : <Hint content="发送"><Button type="button" variant="default" size="icon" onClick={submit} disabled={disabled || (!text.trim() && !pendingWorkItem)} aria-label="发送"><PaperPlaneTilt weight="regular" size={16} /></Button></Hint>}</div></div></div></div>;
}
			...
			<div ref={scrollRef} onScroll={onScroll} className={chatStyles.scroll}>
				<div className={chatStyles.frame}>
					<div ref={contentRef} className={chatStyles.content}>
						{task ? (task.messages.length === 0 && !running ? <div className={chatStyles.empty}><div className={chatStyles.emptyIcon}><Sparkle weight="regular" size={26} /></div><div><h2>开始一项 Work</h2><p>围绕 GitPilot 工作项协同推进工作，正式产出请写入工作空间文件。</p></div></div> : <div className={chatStyles.messages}>
							{task.messages.map((message) => <article key={message.id} className={`${styles.message} ${newMessageId === message.id ? chatStyles.newMessageSlot : ''}`} ref={(node) => {
								...
								{message.sources?.length ? <div className={styles.sources}>{message.sources.map((source) => <a key={source.id} href={source.url} target="_blank" rel="noreferrer"><span className={styles.sourceTitle}>{source.title}</span><span>{source.snippet}</span></a>)}</div> : null}
							</article>)}
...
function WorkFilesInspector({ task, onOpen, onRename, onDelete, collaborationRefreshKey, onSendWorkItemToConversation }: { task: WorkTask | null; onOpen: (file: WorkFile) => void; onRename: (file: WorkFile) => void; onDelete: (file: WorkFile) => void; collaborationRefreshKey: number; onSendWorkItemToConversation: (item: RpcWorkItemDetail) => void }) {
	...
	return <aside className={inspectorStyles.root} data-ui-mode="work" aria-label="工作空间文件">
		<nav className={styles.fileTabs} aria-label="Work 功能页签" onMouseDown={(event) => event.stopPropagation()}>
			<div className={styles.fileTabScroller}>
				{openTabs.map((tabId) => { const tab = tabs.find((item) => item.id === tabId)!; const Icon = tab.icon; return <div key={tab.id} className={`${styles.fileTab} ${activeTab === tab.id ? styles.fileTabActive : ''}`} role="tab" tabIndex={0} aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActiveTab(tab.id); } }}><Icon weight="regular" /><span>{tab.label}</span><button type="button" className={styles.fileTabClose} onClick={(event) => { event.stopPropagation(); closeFunctionTab(tab.id); }} aria-label={`关闭${tab.label}`}><X weight="bold" size={12} /></button></div>; })}
			</div>
			<div className={styles.fileTabActions}>
				<DropdownMenu><Hint content="打开功能页签"><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon-sm" className={`${styles.addFileTab} focus-visible:outline-none focus-visible:ring-0`} aria-label="打开 Work 功能页签"><Plus weight="bold" size={15} /></Button></DropdownMenuTrigger></Hint><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => openFunctionTab('collaboration')}><Network weight="regular" />工作项协同</DropdownMenuItem><DropdownMenuItem onSelect={() => openFunctionTab('files')}><FolderOpen weight="regular" />文件</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
			</div>
		</nav>
		{openTabs.length === 0 ? <div className={styles.fileEmpty}><FolderOpen weight="regular" size={20} /><span>右侧功能面板已关闭</span><small>点击上方“+”重新打开一个功能面板。</small></div> : activeTab === 'files' ? (task ? <ScrollArea className={styles.fileList} fitContent>{task.files.length ? <div className={styles.fileRows} role="list">{task.files.map((file) => { const changeLabel = fileChangeLabel(file.changeState); const isActive = activePath === file.path; return <div key={file.path} className={`${styles.fileRow} ${isActive ? styles.fileRowActive : ''}`} role="listitem"><Button type="button" variant="unstyled" className={styles.fileRowButton} onClick={() => { setActivePath(file.path); onOpen(file); }} aria-label={`打开 ${file.path}`}><span className={`${styles.fileIcon} ${styles[`fileIcon_${fileExtension(file)}`] || ''}`}><FileTypeIcon file={file} /></span><span className={styles.fileBody}><span className={styles.fileName}>{file.name || file.path}</span><span className={styles.fileMeta}><span>{fileExtension(file).toUpperCase()}</span><span>{formatFileSize(file.size)}</span><span>{formatFileDate(file.updatedAt)}</span>{changeLabel ? <span className={styles.fileStatus}>{changeLabel}</span> : null}</span></span></Button><div className={styles.fileActions}><Button type="button" variant="ghost" size="icon-sm" onClick={() => onRename(file)} title="重命名" aria-label={`重命名 ${file.path}`}><PencilSimple weight="regular" size={14} /></Button><Button type="button" variant="ghost" size="icon-sm" className={styles.deleteFileButton} onClick={() => onDelete(file)} title="删除" aria-label={`删除 ${file.path}`}><Trash weight="regular" size={14} /></Button></div></div>; })}</div> : <div className={styles.fileEmpty}><FileText weight="regular" size={20} /><span>暂无文件</span><small>让 Agent 写入工作空间后，文件会出现在这里。</small></div>} </ScrollArea> : <div className={styles.fileEmpty}><FileText weight="regular" size={20} /><span>尚未创建工作空间</span><small>新建任务后即可管理文件产出。</small></div>) : <WorkCollaborationPanel refreshKey={collaborationRefreshKey} onSendToConversation={onSendWorkItemToConversation} />}
	</aside>;
}
...
export function TargetWorkShell() {
	...
	if (!hydrated) return <div className={styles.loading}><CircleNotch weight="bold" className="animate-spin" />正在加载 Work 工作空间…</div>;
