import { File, FileArchive, FileCode2, FileImage, FileJson, FilePlus2, FileSpreadsheet, FileText, Folder, FolderOpen, Loader2, MessageSquarePlus, Network, Pencil, Plus, Send, Sparkles, Square, Trash2, X } from 'lucide-react';
...
function WorkTaskRow({ task, active, running, indent, onSelect }: { task: WorkTask; active: boolean; running: boolean; indent: boolean; onSelect: (id: string) => void }) {
	...
	return <Hint content={label}><Button type="button" variant="ghost" size="sm" data-sidebar-menu-kind="work-task" data-work-task-id={task.id} data-session-cwd={task.workspacePath} aria-busy={running || undefined} onClick={() => onSelect(task.id)} className={`${sidebarStyles.taskRow} ${active ? sidebarStyles.taskActive : ''}`} style={indent ? { paddingLeft: '31px' } : undefined}><span className={sidebarStyles.label}>{label}</span></Button></Hint>;
}

/** 工作空间分组：组头点击切换当前工作空间，新建任务会落到该目录（sidecar cwd 绑定）。 */
function WorkWorkspaceGroup({ workspace, tasks, activeTaskId, runningTaskId, current, onCreate, onSelect, onSelectWorkspace }: { workspace: WorkWorkspaceEntry; tasks: WorkTask[]; activeTaskId: string | null; runningTaskId: string | null; current: boolean; onCreate: (workspacePath?: string | null) => void; onSelect: (id: string) => void; onSelectWorkspace: (path: string | null) => void }) {
...
function WorkTaskSidebar({ tasks, activeTaskId, runningTaskId, workspaces, currentWorkspacePath, onCreate, onSelect, onAddWorkspace, onSelectWorkspace }: WorkTaskSidebarProps) {
	...
	return <aside className={sidebarStyles.root} aria-label="Work 工作空间与任务">
		...
		<header className={sidebarStyles.header}><div className={sidebarStyles.headerCopy}><span>工作空间</span></div><Hint content="添加工作空间"><Button type="button" variant="secondary" size="icon-sm" className={sidebarStyles.headerAction} onClick={onAddWorkspace} aria-label="添加工作空间"><Plus aria-hidden="true" /></Button></Hint></header>
		<ScrollArea fitContent className={sidebarStyles.scroll}><div className={sidebarStyles.content}>
			{workspaces.map((workspace) => <WorkWorkspaceGroup key={workspace.path} workspace={workspace} tasks={tasks.filter((task) => task.workspaceRootPath === workspace.path)} activeTaskId={activeTaskId} runningTaskId={runningTaskId} current={workspace.path === currentWorkspacePath} onCreate={onCreate} onSelect={onSelect} onSelectWorkspace={onSelectWorkspace} />)}
...
/** Work 输入器在欢迎页回到文档流，避免默认悬浮层覆盖欢迎标题；消息态继续使用悬浮定位。 */
function WorkInputBox({ disabled, running, onSend, onAbort, inline = false }: { disabled: boolean; running: boolean; onSend: (text: string) => void; onAbort: () => void; inline?: boolean }) {
...
function WorkConversation({ task, streamingText, running, inputBlocked, onSend, onAbort }: { task: WorkTask | null; streamingText: string; running: boolean; inputBlocked: boolean; onSend: (text: string) => void; onAbort: () => void }) {
	...
	if (isEmpty) return <main className={styles.conversation} aria-label="Work 对话区">
		<WelcomeView mode="work" composer={<WorkInputBox disabled={inputBlocked} running={running} onSend={onSend} onAbort={onAbort} inline />} />
	</main>;
			...
			<div ref={scrollRef} onScroll={onScroll} className={chatStyles.scroll}>
				<div className={chatStyles.frame}>
					<div ref={contentRef} className={chatStyles.content}>
						{task ? (task.messages.length === 0 && !running ? <div className={chatStyles.empty}><div className={chatStyles.emptyIcon}><Sparkles size={26} /></div><div><h2>开始一项 Work</h2><p>围绕 GitPilot 公众端协同推进工作，正式产出请写入工作空间文件。</p></div></div> : <div className={chatStyles.messages}>
...
export function TargetWorkShell() {
	...
	const send = async (text: string) => {
		...
		try {
			...
			appendMessage(task.id, { id: id(), role: 'assistant', text: response.data.text, createdAt: Date.now() });
		} catch (error) {
