	...
	return (
		<section className={styles.card} role="alert" aria-label="安全审批">
			...
			<div className={styles.actions}>
				<Button size="sm" onClick={() => void respond(approval, 'approve_once')}><Check weight="bold" size={13} />允许一次</Button>
				<Button size="sm" variant="secondary" onClick={() => void respond(approval, 'approve_session')}><Check weight="bold" size={13} />本会话允许</Button>
				<Button size="sm" variant="ghost" onClick={() => void respond(approval, 'deny')}><X weight="bold" size={13} />拒绝</Button>
			</div>
		</section>
	);
}
