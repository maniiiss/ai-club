...
import { Folder, FolderOpen, LoaderCircle, Maximize2, MessageSquarePlus, Minimize2, Pencil, Plus } from 'lucide-react';
		...
		<div className={styles.projectBlock} data-sidebar-menu-kind="project" data-project-path={node.project.path} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
			...
			{node.tasks.length === 0 ? <div className={styles.emptyProject} style={{ paddingLeft: `${39 + depth * 14}px` }}>暂无工作空间任务</div> : <div className={styles.taskList}>{node.tasks.map((task) => {
				const label = task.name || task.firstMessage || '未命名工作空间任务';
					return <Hint key={task.path} content={task.isStreaming ? `${label}（进行中）` : label}><Button type="button" variant="ghost" size="sm" data-sidebar-menu-kind="project-task" data-session-path={task.path} data-session-cwd={task.cwd} aria-busy={task.isStreaming || undefined} onClick={() => onSelectTask(task.path)} className={cn(styles.taskRow, task.path === activeTaskPath ? styles.taskActive : '')} style={{ paddingLeft: `${31 + depth * 14}px` }}><span className={styles.label}>{label}</span>{task.isStreaming && <LoaderCircle className={styles.taskLoading} aria-hidden="true" />}</Button></Hint>;
				})}</div>}
			</CollapsibleContent>
		</div>
	</Collapsible>;
...
export function TargetSessionSidebar() {
	...
	<header className={styles.header}><div className={styles.headerCopy}><span>工作空间</span></div><div className={styles.headerActions}>{allProjectsCollapsed ? <Hint content="展开全部工作空间"><Button type="button" variant="secondary" size="icon-sm" className={styles.headerAction} onClick={expandAllProjects} disabled={projectTree.length === 0} aria-label="展开全部工作空间"><Maximize2 aria-hidden="true" /></Button></Hint> : <Hint content="收起全部工作空间"><Button type="button" variant="secondary" size="icon-sm" className={styles.headerAction} onClick={collapseAllProjects} disabled={!hasExpandedProject} aria-label="收起全部工作空间"><Minimize2 aria-hidden="true" /></Button></Hint>}<Hint content="添加工作空间"><Button type="button" variant="secondary" size="icon-sm" className={styles.headerAction} onClick={() => void addProject()} disabled={connection !== 'ready'} aria-label="添加工作空间"><Plus aria-hidden="true" /></Button></Hint></div></header>
		<ScrollArea fitContent className={styles.scroll}><div className={styles.content}>
			...
			<div className={styles.standaloneList}>{standaloneTasks.length === 0 ? <div className={styles.emptyTask}>暂无独立任务</div> : standaloneTasks.map((session) => { const active = session.path === currentFile; const label = session.name || session.firstMessage || '未命名任务'; return <Hint key={session.path} content={session.isStreaming ? `${label}（进行中）` : label}><Button type="button" variant="ghost" size="sm" data-sidebar-menu-kind="standalone-task" data-session-path={session.path} data-session-cwd={session.cwd} aria-busy={session.isStreaming || undefined} onClick={() => void switchSession(session.path)} className={cn(styles.taskRow, active ? styles.taskActive : '')}><span className={styles.label}>{label}</span>{session.isStreaming && <LoaderCircle className={styles.taskLoading} aria-hidden="true" />}</Button></Hint>; })}</div>
		</div></ScrollArea>
	</aside>;
}
