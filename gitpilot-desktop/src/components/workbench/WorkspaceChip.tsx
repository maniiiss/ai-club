...
export function WorkspaceChip({ items, currentPath, onSelect, onAdd }: WorkspaceChipProps) {
	...
	return <DropdownMenu>
		...
		<DropdownMenuContent align="center" className={styles.menu}>
			{items.length === 0 ? <DropdownMenuItem disabled>暂无工作空间</DropdownMenuItem> : items.map((item) => <DropdownMenuItem key={item.path} onSelect={() => onSelect(item.path)}><FolderOpen size={14} aria-hidden="true" /><span className={styles.itemCopy}><span>{item.name}</span><small>{item.path}</small></span>{item.path === currentPath && <Check size={13} aria-label="当前工作空间" />}</DropdownMenuItem>)}
