	...
	return <Popover open={open} onOpenChange={(value) => value ? setOpen(true) : close()}>
		<Hint content={label}>
			<PopoverTrigger asChild>
				<Button type="button" variant="ghost" size={triggerVariant === 'nav' ? 'sm' : 'icon-sm'} className={cn(styles.trigger, triggerVariant === 'nav' ? styles.navTrigger : '', className)} aria-label={label}><Search aria-hidden="true" />{triggerVariant === 'nav' && <span>{triggerText ?? label}</span>}</Button>
			</PopoverTrigger>
		</Hint>
		<PopoverContent side="bottom" align="end" className={styles.popover} onOpenAutoFocus={(event) => event.preventDefault()}>
			<div className={styles.root}>
				<div className={styles.heading}><span><Search aria-hidden="true" />{label}</span><button type="button" className={styles.close} onClick={close} aria-label="关闭搜索"><X aria-hidden="true" /></button></div>
				<div className={styles.inputRow}><Search aria-hidden="true" /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') close(); }} placeholder={placeholder} aria-label={placeholder} /></div>
				{!query.trim() ? <div className={styles.empty}>输入关键词，搜索标题和历史消息</div> : results.length === 0 ? <div className={styles.empty}>未找到匹配的历史任务</div> : <div className={styles.results} role="listbox" aria-label="历史任务搜索结果">{results.map((result) => { const formattedTime = formatUpdatedAt(result.updatedAt); return <button key={result.key} type="button" className={styles.result} role="option" onClick={() => select(result)}><span className={styles.resultIcon}><Clock3 aria-hidden="true" /></span><span className={styles.resultCopy}><strong>{result.title}</strong><span>{result.summary}</span></span>{formattedTime && <time dateTime={String(result.updatedAt)}>{formattedTime}</time>}</button>; })}</div>}
			</div>
		</PopoverContent>
	</Popover>;
}
