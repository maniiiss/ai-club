...
import { CheckCircle, Circle, FileMagnifyingGlass, GitBranch, GitDiff, ListChecks, PencilSimple, Play, Plus, Terminal, TreeStructure, Warning, Wrench, X } from '@phosphor-icons/react';
...
import { TargetGitPanel } from './TargetGitPanel';
...
export function TargetExecutionInspector({ showExecution = true }: TargetExecutionInspectorProps) {
	...
	const openGitPanelTab = useWorkbenchStore((s) => s.openGitPanelTab);
	...
	const activeTabId = rightPanelTabs.activeTabId === 'files' && rightPanelTabs.filesOpen
		...
		: rightPanelTabs.activeTabId === 'review' && rightPanelTabs.reviewOpen
			? 'review'
			: rightPanelTabs.activeTabId === 'git' && rightPanelTabs.gitOpen
				? 'git'
	...
	return <aside className={styles.root} data-ui-mode={showExecution ? 'execution' : 'code'} aria-label={showExecution ? '执行过程' : '右侧工具窗口'}>
		<nav className={styles.tabs} aria-label="右侧工具窗口页签" onMouseDown={(event) => event.stopPropagation()}>
			<div className={styles.tabScroller}>
				...
				{rightPanelTabs.gitOpen && <div data-tab-id="git" className={`${styles.tab} ${activeTabId === 'git' ? styles.tabActive : ''}`} role="tab" tabIndex={0} aria-selected={activeTabId === 'git'} onClick={() => activateRightPanelTab('git')} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activateRightPanelTab('git'); } }}><GitBranch weight="regular" size={15} /><span>Git</span><button type="button" className={styles.tabClose} onClick={(event) => { event.stopPropagation(); closeRightPanelTab('git'); }} aria-label="关闭 Git"><X weight="bold" size={13} /></button></div>}
			...
			<div className={styles.tabActions}>
				<DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon-sm" className={`${styles.addTab} focus-visible:outline-none focus-visible:ring-0`} aria-label="打开右侧工具窗口" data-gp-hint="打开工具窗口"><Plus weight="bold" size={16} /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{showExecution && <DropdownMenuItem onSelect={() => openExecutionPanelTab()}><Circle weight="regular" />执行过程</DropdownMenuItem>}<DropdownMenuItem onSelect={() => openProjectFilesPanel()}><TreeStructure weight="regular" />文件</DropdownMenuItem><DropdownMenuItem onSelect={() => openGitPanelTab()}><GitBranch weight="regular" />Git</DropdownMenuItem><DropdownMenuItem onSelect={() => openReviewPanelTab()}><GitDiff weight="regular" />审查</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
			</div>
		</nav>
		{activePlan ? <PlanPanel plan={activePlan} /> : activeTabId === 'files' && rightPanelTabs.filesOpen ? <TargetProjectFilesPanel /> : activeTabId === 'git' && rightPanelTabs.gitOpen ? <TargetGitPanel /> : activeTabId === 'review' && rightPanelTabs.reviewOpen ? <TargetReviewPanel /> : showExecution && activeTabId === 'execution' ? <>
			<ScrollArea className={styles.timeline} aria-label="Agent 执行时间线">{execution?.steps.length === 0 ? <div className={styles.empty}><Play weight="regular" size={18} /><span>任务开始后，工具调用会在这里按真实顺序展开。</span></div> : execution?.steps.map((step) => <Button key={step.id} type="button" variant="unstyled" size="sm" onClick={() => selectStep(step.id)} className={`${styles.step} ${selected?.id === step.id ? styles.selected : ''} h-auto min-h-[48px] focus-visible:outline-none`}><span className={`${styles.stepIcon} ${styles[`status_${step.status}`]}`}><StepIcon step={step} /></span><span className={styles.stepBody}><span className={styles.stepTitle}>{step.title}</span><small>{step.kind} · {duration(step)}</small></span><span className={styles.stepState}>{step.status === 'waiting' ? '待确认' : step.status === 'running' ? '运行中' : step.status === 'failed' ? '失败' : '完成'}</span></Button>)}</ScrollArea>
