...
export function McpSettingsPanel() {
	...
	const fields = draft.transport === 'stdio' ? <>
		...
		<label className="grid gap-1 text-xs text-[var(--secondary-foreground)]">参数（空格分隔）<Input value={draft.args} onChange={(event) => updateDraft({ args: event.target.value })} placeholder="-y @modelcontextprotocol/server-memory" /></label>
		<label className="grid gap-1 text-xs text-[var(--secondary-foreground)] sm:col-span-2">环境变量（可选 JSON）<textarea key={`env-${editingName ?? 'new'}-${sensitiveInputRevision}`} className={fieldClass} rows={4} defaultValue={sensitiveInputs.current.env} onChange={(event) => { sensitiveInputs.current.env = event.target.value; }} placeholder={'{\n  "MY_API_KEY": "your-key"\n}'} /></label>
	</> : <>
		<label className="grid gap-1 text-xs text-[var(--secondary-foreground)] sm:col-span-2">URL<textarea className={`${fieldClass} resize-y`} rows={1} value={draft.url} onChange={(event) => updateDraft({ url: event.target.value })} placeholder="https://mcp.example.com/mcp" /></label>
		<label className="grid gap-1 text-xs text-[var(--secondary-foreground)] sm:col-span-2">请求头（可选 JSON）<textarea key={`headers-${editingName ?? 'new'}-${sensitiveInputRevision}`} className={fieldClass} rows={4} defaultValue={sensitiveInputs.current.headers} onChange={(event) => { sensitiveInputs.current.headers = event.target.value; }} placeholder={'{\n  "Authorization": "Bearer your-token"\n}'} /></label>
	</>;
	...
	return <div className="flex min-h-0 flex-1 flex-col">
		<div className="border-b border-[var(--border)] px-5 py-3"><div className="flex items-center gap-2"><Button type="button" variant="ghost" size="sm" onClick={backToList} aria-label="返回 MCP 服务列表" title="返回 MCP 服务列表"><ArrowLeft />返回</Button><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-semibold text-[var(--foreground)]">{editingServer ? `编辑 ${editingServer.name}` : '新建全局服务'}</h3></div><Button type="button" size="sm" onClick={() => void save()} disabled={busy}>{busy ? '保存中…' : <><Save />保存</>}</Button></div></div>
		<div className="min-h-0 flex-1 overflow-y-auto px-5 py-4"><div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className="mb-1 block text-xs font-medium text-[var(--secondary-foreground)]">传输类型</span><div className="flex rounded-[var(--radius-sm)] border border-[var(--border)] p-0.5"><Button type="button" variant={draft.transport === 'stdio' ? 'secondary' : 'ghost'} size="sm" onClick={() => updateDraft({ transport: 'stdio' })}>stdio</Button><Button type="button" variant={draft.transport === 'http' ? 'secondary' : 'ghost'} size="sm" onClick={() => updateDraft({ transport: 'http' })}>HTTP</Button><Button type="button" variant={draft.transport === 'sse' ? 'secondary' : 'ghost'} size="sm" onClick={() => updateDraft({ transport: 'sse' })}>SSE</Button></div></div><div><span className="mb-1 block text-xs font-medium text-[var(--secondary-foreground)]">编辑模式</span><div className="flex rounded-[var(--radius-sm)] border border-[var(--border)] p-0.5"><Button type="button" variant={!jsonMode ? 'secondary' : 'ghost'} size="sm" onClick={() => toggleJsonMode(false)}>表单</Button><Button type="button" variant={jsonMode ? 'secondary' : 'ghost'} size="sm" onClick={() => toggleJsonMode(true)}>JSON</Button></div></div></div><div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-xs text-[var(--secondary-foreground)]">名称<Input value={draft.name} onChange={(event) => updateDraft({ name: event.target.value })} placeholder="my-mcp-server" aria-label="MCP 服务名" /></label><label className="grid gap-1 text-xs text-[var(--secondary-foreground)]">超时时间 MS<Input type="number" value={draft.timeout} onChange={(event) => updateDraft({ timeout: event.target.value })} placeholder={String(DEFAULT_TIMEOUT)} /></label>{jsonMode ? <label className="grid gap-1 text-xs text-[var(--secondary-foreground)] sm:col-span-2">MCP 服务定义<textarea className={`${fieldClass} min-h-48 font-mono`} defaultValue={jsonDefinition.current} onChange={(event) => { jsonDefinition.current = event.target.value; }} spellCheck={false} /></label> : fields}</div><div className="border-t border-[var(--border)] pt-4"><span className="mb-2 block text-xs font-medium text-[var(--secondary-foreground)]">作用域</span>{modeSelector}</div>{error && <p role="alert" className="text-xs text-[var(--destructive)]">{error}</p>}<p className="text-[10px] text-[var(--muted-foreground)]">环境变量和请求头只显示脱敏占位符；未修改的凭据由 sidecar 保留。</p></div></div>
	</div>;
}
