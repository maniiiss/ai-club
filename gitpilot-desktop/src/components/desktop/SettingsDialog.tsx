...
function DesktopUpdatePanel() {
	...
	return <div className={styles.updateBody}>
		...
		{update && <article className={styles.updateCard}>
			...
			{confirming ? <div className={styles.updateConfirm}><span className={styles.updateConfirmTitle}>确认下载并安装 GitPilot {update.version}？</span><span>下载完成后会校验签名、替换应用并自动重启；本地会话数据会保留。</span><div><Button type="button" variant="outline" size="sm" onClick={() => setConfirming(false)}>稍后</Button><Button type="button" size="sm" disabled={busy || downloading} onClick={() => void handleInstall()}><ShieldCheck />确认安装</Button></div></div> : <Button type="button" size="sm" disabled={busy || downloading} onClick={() => setConfirming(true)}>{downloading ? (progress === 100 ? '正在重启…' : `下载中 ${progress ?? 0}%`) : '下载并安装'}</Button>}
...
function BasicSettings({ draft, dirty, directoryError, onChange, onChooseDirectory, onClearDirectory, onDiscard, onSave }: BasicSettingsProps) {
	...
	return <>
		<div className={styles.basicBody}>
			...
			<section className={styles.section}><div className={styles.sectionHeading}><h3>独立任务默认目录</h3></div><div className={styles.directoryRow}><div className={`${styles.directoryValue} ${!draft.defaultDirectory ? styles.directoryEmpty : ''}`} title={draft.defaultDirectory ?? undefined}>{draft.defaultDirectory ?? '未设置，将使用 GitPilot 根目录'}</div><Hint content="选择默认目录"><Button type="button" variant="outline" size="icon-sm" onClick={onChooseDirectory} aria-label="选择默认目录"><FolderOpen /></Button></Hint>{draft.defaultDirectory && <Hint content="清除默认目录"><Button type="button" variant="ghost" size="icon-sm" onClick={onClearDirectory} aria-label="清除默认目录"><X /></Button></Hint>}</div>{directoryError && <p role="alert" className="mt-2 text-[length:var(--text-[length:var(--text-xs)])] text-[var(--destructive)]">{directoryError}</p>}</section>
		</div>
