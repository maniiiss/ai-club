...
import { Briefcase, Code, Copy, Minus, Palette, Square, X } from '@phosphor-icons/react';
...
export function TargetTitleBar() {
	...
	return (
		<header className={styles.titlebar} data-mode={mode} onMouseDown={startDragging}>
				...
				<nav className={styles.modeSwitcher} aria-label="GitPilot 模式" onMouseDown={(event) => event.stopPropagation()}>
					{MODE_META.map(({ key, label, Icon }) => {
						...
						return <button key={key} type="button" className={active ? styles.modeActive : styles.mode} aria-current={active ? 'page' : undefined} onClick={() => setMode(key)}>
							<Icon weight="regular" className={styles.modeIcon} aria-hidden="true" />
							<span>{label}</span>
						</button>;
					})}
				</nav>
			...
			<div className={styles.actions} onMouseDown={(event) => event.stopPropagation()}>
				...
				<Hint content="最小化"><Button variant="ghost" size="icon-sm" onClick={() => runWindowAction('最小化', minimizeWindow)} aria-label="最小化"><Minus weight="bold" /></Button></Hint>
				<Hint content={isMaximized ? '还原' : '最大化'}>
					...
					>
						{isMaximized ? <Copy weight="regular" /> : <Square weight="regular" />}
					</Button>
				</Hint>
				<Hint content="关闭"><Button variant="ghost" size="icon-sm" className={styles.close} onClick={() => runWindowAction('关闭', closeWindow)} aria-label="关闭"><X weight="bold" /></Button></Hint>
			</div>
		</header>
	);
}
