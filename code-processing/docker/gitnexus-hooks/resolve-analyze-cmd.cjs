/**
 * Docker 运行时补丁：GitNexus 1.6.7 官方 CLI 镜像遗漏了 hooks 目录，
 * 会导致 analyze/query/context/serve 在加载 resolve-invocation.js 时崩溃。
 * 这里先补齐上游 hooks/claude/resolve-analyze-cmd.cjs，同步保持原始导出结构，
 * 让 code-processing 容器在上游镜像修复前仍能稳定执行仓库代码结构化链路。
 */
...
// Version-probe timeout, kept under Claude Code's 10s hook budget.
// PATH presence detection is now spawn-free (resolveOnPath scans PATH directly),
// so the only subprocesses left are the version probes: in a linked worktree the
// stale-index hook first runs `git rev-parse --git-common-dir` (~2s) and
// `git rev-parse HEAD` (~3s); the pnpm path then adds up to two 1s `--version`
// probes (npm, pnpm), so the worst case is ~7s — within budget. A healthy
// `--version` returns in well under a second, so the realistic cost is far lower.
 ...
 * spawn timeout to tune.
 *
 * On Windows it matches PATHEXT extensions ONLY — exactly what `where`/cmd.exe
 * resolve — so neither an un-spawnable `.ps1`-only shim (not in default PATHEXT)
 * nor a bare extensionless file (which the shell cannot launch as `command`) is
 * a false positive. `preferExecExt` returns a recognized `.cmd`/`.bat`/`.exe`
 * shim ahead of an exotic PATHEXT hit (e.g. `.COM`) when both match, matching
 * what a user would actually launch. Pure (platform/env injectable) so it is
 * unit-testable without touching the host PATH.
 */
function resolveOnPath(command, preferExecExt = false, { platform = process.platform, env = process.env } = {}) {
...
// One spawn of `command --version` → { major, minor } (each null when unreadable).
// Version injection happens at the resolver seam (getNpmMajorVersion /
...
function formatPnpmAllowBuildArgs(options = {}, deps = {}) {
  if (!options.alwaysAllowBuild) {
    const { major, minor } =
      'pnpmMajor' in deps
        ? {
            major: deps.pnpmMajor,
            minor: 'pnpmMinor' in deps ? deps.pnpmMinor : null,
          }
        : probeVersion('pnpm');

    const lacksAllowBuild = major !== null && (major < 10 || (major === 10 && minor !== null && minor < 2));
    if (lacksAllowBuild) return [];
  }

  const pkgs = [...PNPM_ALLOW_BUILD_BASE];
  if (options.embeddings) pkgs.push(...PNPM_ALLOW_BUILD_EMBEDDINGS);
  return pkgs.map((pkg) => `--allow-build=${pkg}`);
}
...
function resolveInvocationMode(probe = resolveOnPath, deps = {}) {
  ...
  const hasPnpm =
    'pnpmPresent' in deps ? deps.pnpmPresent : 'pnpmMajor' in deps ? deps.pnpmMajor !== null : Boolean(probe('pnpm'));
...
function formatAnalyzeCommand(options = {}, deps = {}) {
  ...
  // unreadable. Injected deps (tests) and forced/global modes skip the pnpm
  // probe.
  const cache = new Map();
...
function buildRunnerArgv(mode, gitnexusArgs, deps = {}) {
  ...
  const embeddings = gitnexusArgs.some((arg) => arg === '--embeddings' || arg.startsWith('--embeddings='));
...
// Direct-exec entrypoint (#1945): `node run.cjs <args>` resolves the best
// available runner (global `gitnexus` → `pnpm dlx` → `npx`) at call time and
...
// CLI-neutral command without baking in a package-manager assumption.
// `gitnexus analyze` drops a copy of this file at `.gitnexus/run.cjs`.
// Skipped on require() (the CLI and tests reuse the exports above), so it runs
