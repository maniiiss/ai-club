...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".gitpilot", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".gitpilot", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".gitpilot", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".gitpilot"), { recursive: true });
			writeFileSync(join(cwd, ".gitpilot", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".gitpilot"), { recursive: true, force: true });
