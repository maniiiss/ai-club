...
/**
 * 从 canonical 页面入口路径提取页面标识。
 * 业务意图：新增页面的事实来源是 pages/<pageId>/index.html，不能依赖
 * 当前运行上下文中的 active pageId，否则模型创建新页面时会被误判为当前页文件。
 */
export function designPageIdFromEntryPath(path: string): string | undefined {
	const match = path.match(/^pages\/([a-zA-Z0-9_-]+)\/index\.html$/);
	return match?.[1];
}
...
export function synchronizeDesignPages(
	...
	for (const file of files) {
		...
		if (!match || !designPageIdFromEntryPath(file.path) || pageIds.has(match[1])) continue;
		pageIds.add(match[1]);
