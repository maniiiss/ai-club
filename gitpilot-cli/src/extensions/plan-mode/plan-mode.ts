...
export default function planMode(
	...
	pi.on("session_start", async (event, ctx) => {
		// resume（切回既有会话）时保留 pending 菜单/确认弹框：既不 abort menuController 也不递增
		// menuGeneration，否则切换会话会把用户待响应的 plan 确认 select 当作旧菜单取消，导致
		// agent 未确认即继续、且响应因 sessionGeneration !== menuGeneration 被判 stale 丢弃。
		// 仅 new/fork/startup/reload 等真正新会话才重置菜单代次。
		const isResume = event.reason === "resume";
		const generation = isResume ? menuGeneration : ++menuGeneration;
		if (!isResume) {
	...
	async function showPlanMenu(ctx: ExtensionContext) {
		...
		await showPlanModeMenu(ctx, {
			...
			refine: (feedback: string) => {
				// 用户在浮层"其他"输入框提交的自定义反馈：连同原计划发给 AI 修改。
				// sendPlanModeUserMessage 触发新回合，before_agent_start 会清 state.latestPlan，
				// 但原计划已随消息发给 AI；AI 修改后 plan_mode_complete -> showReadyPlanMenu 再弹，plan 模式不打断。
	...
	async function showPlanReadyMenu(ctx: ExtensionContext) {
		...
		await showReadyPlanMenu(ctx, {
			...
			refine: (feedback: string) => {
				// 用户在浮层"其他"输入框提交的自定义反馈：连同原计划发给 AI 修改，plan 模式继续。
				const plan = state.latestPlan?.trim();
				sendPlanModeUserMessage(
					`用户对当前计划的反馈：${feedback}\n\n当前计划：\n${plan ?? "（无）"}\n\n请根据反馈修改计划，完成后再次调用 plan_mode_complete 提交新计划。`,
					ctx,
