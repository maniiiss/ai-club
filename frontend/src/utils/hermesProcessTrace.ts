export type HermesProcessTone = 'running' | 'success' | 'warning' | 'danger' | 'muted'

export interface HermesToolStatusMeta {
  statusText: string
  tone: HermesProcessTone
  icon: string
}

export interface HermesToolExecutionViewItem extends HermesToolStatusMeta {
  id: string
  toolName: string
  compactLabel: string
  functionName: string
...
export interface HermesToolTraceSummary {
  title: string
  description: string
  countText: string
  tone: HermesProcessTone
...
const HERMES_TOOL_DISPLAY_NAMES: Record<string, string> = {
  'project.search': '搜索项目',
...
const resolveHermesToolDisplayName = (toolCode: string, functionName: string) => {
  const normalizedToolCode = toolCode.trim()
  const normalizedFunctionName = functionName.trim()
  return HERMES_TOOL_DISPLAY_NAMES[normalizedToolCode]
    || HERMES_TOOL_DISPLAY_NAMES[normalizedFunctionName]
    || normalizedToolCode
...
/**
 * 将 Hermes 后端调试状态收敛成前端可读语义，避免模板里散落状态字符串判断。
 */
export const resolveHermesToolStatusMeta = (status?: string | null): HermesToolStatusMeta => {
...
export const normalizeHermesToolExecutions = (toolExecutions: Array<Record<string, unknown>> = []): HermesToolExecutionViewItem[] =>
  toolExecutions.map((item, index) => {
    ...
    const meta = resolveHermesToolStatusMeta(rawStatus)
    const fallbackId = `${toolCode || functionName || 'tool'}-${index}`
    const toolName = resolveHermesToolDisplayName(toolCode, functionName)
    return {
      ...meta,
...
export const buildHermesToolTraceSummary = (
  items: HermesToolExecutionViewItem[]
): HermesToolTraceSummary | null => {
  if (!items.length) {
    return null
  }
  ...
  const tone: HermesProcessTone = failedCount > 0
    ? 'danger'
    : warningCount > 0
      ? 'warning'
      : runningCount > 0
...
export const buildHermesStreamingThinkMarkdown = (statusText?: string | null) =>
  `<think>${(statusText || '').trim() || 'Hermes 正在思考'}`
