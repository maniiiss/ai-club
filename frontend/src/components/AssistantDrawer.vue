            ...
            <div v-for="message in currentMessages" :key="message.id" class="hermes-message-row" :class="message.role">
              ...
              <div class="hermes-message-bubble" :class="[message.status, { 'stream-loading': shouldShowInlineStreamStatus(message) && !message.content?.trim() }]">
                ...
                <template v-else>
                  ...
                  <div v-if="shouldShowInlineStreamStatus(message)" class="assistant-thinking-indicator" :class="{ compact: Boolean(message.content?.trim()) }">
                    <span class="assistant-thinking-icon" aria-hidden="true"></span>
...
import { archiveAssistantConversationSession, clearAssistantUserMemories, consolidateAssistantUserMemories, createAssistantConversationSession, deleteAssistantConversationSession, deleteAssistantFileLibraryItem, deleteAssistantUserMemory, getAssistantConversationDetail, getAssistantMemoryConsolidationStatus, listAssistantFileLibraryItems, listAssistantUserMemories, markAssistantActionExecuted, pageAssistantConversationSessions, reindexAssistantFileLibraryItem, renameAssistantConversationSession, restoreAssistantConversationSession, streamAssistantSessionChat, streamAssistantSessionChatWithFiles, transcribeAssistantSpeech, updateAssistantFileLibraryItem, uploadAssistantFileLibraryItem } from '@/api/assistant'
import { createGitlabBindingScanTask } from '@/api/gitlab'
import { createExecutionTask, createTask, createTestPlan } from '@/api/platform'
import { renderAssistantMarkdownToHtml } from '@/utils/assistantMarkdown'
import { resolveAssistantDrawerPresentation } from '@/utils/assistantDrawerLayout'
import { buildAssistantToolTraceSummary, normalizeAssistantToolExecutions } from '@/utils/assistantProcessTrace'
import { DEFAULT_REQUIREMENT_TEMPLATE } from '@/utils/requirementTemplate'
import type { CreateAssistantConversationSessionPayload, AssistantActionItem, AssistantAttachmentItem, AssistantConversationDetailItem, AssistantConversationSessionSummaryItem, AssistantDebugInfoItem, AssistantFileLibraryItem, AssistantMemoryConsolidationStatus, AssistantMemoryFactItem, AssistantMessageItem, AssistantReferenceItem, AssistantSelectionCardItem, AssistantSelectionOptionItem, AssistantSelectionPayload, AssistantSessionChatRequestPayload, AssistantStreamDeltaEvent, AssistantStreamDoneEvent, AssistantStreamErrorEvent, AssistantStreamMetaEvent, AssistantStreamStatusEvent, AssistantUserMemoryItem } from '@/types/assistant'
import type { AssistantToolExecutionViewItem, AssistantToolTraceSummary } from '@/utils/assistantProcessTrace'
...
interface AssistantQuestionInputExpose {
  focus?: () => void
}

const props = defineProps<AssistantDrawerProps>()
const drawerVisible = defineModel<boolean>({ default: false })
const router = useRouter()
const messageScrollRef = ref<HTMLDivElement>()
const questionInputRef = ref<AssistantQuestionInputExpose | null>(null)
...
const sessionSummaries = ref<AssistantConversationSessionSummaryItem[]>([])
...
const currentSessionDetail = ref<AssistantConversationDetailItem | null>(null)
const currentMessages = ref<AssistantMessageItem[]>([])
const currentReferences = ref<AssistantReferenceItem[]>([])
const currentSuggestions = ref<string[]>([])
const currentActions = ref<AssistantActionItem[]>([])
const currentSelectionCards = ref<AssistantSelectionCardItem[]>([])
const currentDebug = ref<AssistantDebugInfoItem | null>(null)
...
let assistantDrawerDisposed = false
...
const memoryList = ref<AssistantUserMemoryItem[]>([])
const memoryFactList = ref<AssistantMemoryFactItem[]>([])
...
const fileLibraryItems = ref<AssistantFileLibraryItem[]>([])
...
// 统一把 Assistant 返回的引用类型转成前端可读的中文标签，避免直接暴露后端枚举值。
const ASSISTANT_REFERENCE_TYPE_LABELS: Record<string, string> = {
  DASHBOARD: '首页看板',
  EXECUTION_TASK: '执行任务',
...
/**
 * 将 Assistant 的能力边界与确认机制收口到标题旁提示里，避免在正文区重复铺开长文案。
 */
const ASSISTANT_TOOLTIP_COPY = {
  intro: 'GitPilot 是平台内协作助手，会优先结合当前页面上下文和平台内可用数据，协助查询项目、工作项、执行任务、测试计划、Wiki 和附件内容。',
...
const assistantTooltipCopy = computed(() => ASSISTANT_TOOLTIP_COPY)
...
const currentStreamStatusText = computed(() => {
  ...
  if (stage === 'connecting') {
    return explicitMessage || 'GitPilot 正在连接服务'
  }
  if (idleMs >= 4500) {
    return hasVisibleContent ? 'GitPilot 正在调用工具补充细节' : 'GitPilot 正在调用工具收集信息'
  }
  if (!hasVisibleContent) {
    if (stage === 'thinking' && elapsedMs >= 2200) {
      return 'GitPilot 正在分析问题'
    }
      return explicitMessage || 'GitPilot 正在准备回答'
  }
  if (idleMs >= 2200) {
    return 'GitPilot 正在整理已收集的信息'
  }
  if (stage === 'thinking') {
    return 'GitPilot 正在整理回答'
  }
  return explicitMessage || 'GitPilot 正在整理回答'
})
...
// 底部输入区的禁用状态会被快捷指令菜单和多个操作按钮共同使用，必须先于这些计算属性初始化。
...
const drawerPresentation = computed(() => resolveAssistantDrawerPresentation({
...
onMounted(() => {
  assistantDrawerDisposed = false
  syncViewportMode()
  voiceInputSupported.value = resolveVoiceInputSupport()
  if (typeof window !== 'undefined') {
    isDebugMode.value = window.localStorage.getItem(ASSISTANT_DEBUG_STORAGE_KEY) === '1'
    window.addEventListener('resize', syncViewportMode)
  }
})

onBeforeUnmount(() => {
  assistantDrawerDisposed = true
  stopVoiceRecording(true)
...
const loadSessionList = async (reset = false) => {
  ...
  try {
    const pageData = await pageAssistantConversationSessions({ page: reset ? 1 : sessionPage.value + 1, size: SESSION_PAGE_SIZE, archived: archivedView.value })
...
const loadMemoryList = async (query?: string) => {
  ...
  try {
    const overview = await listAssistantUserMemories(query || '', 50)
    memoryList.value = overview.conversationMemories || []
    memoryFactList.value = overview.consolidatedFacts || []
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || '加载 GitPilot 记忆失败')
  } finally {
...
/**
 * 加载个人文件库列表，文件库与会话记忆共同组成 Assistant 的个人知识入口。
 */
const loadFileLibraryItems = async (query?: string) => {
  fileLibraryLoading.value = true
  try {
    fileLibraryItems.value = await listAssistantFileLibraryItems(query)
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || '加载 GitPilot 文件库失败')
  } finally {
...
const handleFileLibraryInputChange = async (event: Event) => {
  ...
  try {
    ...
    ElMessage.success('文件已加入 GitPilot 文件库')
...
const handleToggleFileLibraryItem = async (item: AssistantFileLibraryItem) => {
...
const handleReindexFileLibraryItem = async (item: AssistantFileLibraryItem) => {
  fileLibraryUpdatingId.value = item.id
  try {
...
const handleDeleteFileLibraryItem = async (item: AssistantFileLibraryItem) => {
  try {
    await ElMessageBox.confirm('确定删除这个文件库条目吗？删除后不可恢复。', '删除文件', { type: 'warning' })
    fileLibraryUpdatingId.value = item.id
    await deleteAssistantFileLibraryItem(item.id)
...
const handleDownloadFileLibraryItem = async (item: AssistantFileLibraryItem) => {
  try {
 ...
 * 删除单条记忆。
 */
const handleDeleteMemory = async (memory: AssistantUserMemoryItem) => {
  try {
    await ElMessageBox.confirm('确定删除这条记忆吗？删除后不可恢复。', '删除记忆', { type: 'warning' })
    await deleteAssistantUserMemory(memory.documentId)
...
const handleClearMemories = async () => {
  try {
    await ElMessageBox.confirm('确定清空全部 GitPilot 记忆吗？此操作不可恢复。', '清空记忆', { type: 'warning' })
    const deletedCount = await clearAssistantUserMemories()
 ...
 */
const resolveMemoryConsolidationStatusText = (status: AssistantMemoryConsolidationStatus) => {
...
const waitForMemoryConsolidation = async (operationId: string) => {
  for (let attempt = 0; attempt < 90; attempt += 1) {
    const status = await getAssistantMemoryConsolidationStatus(operationId)
...
const createAndSelectSession = async () => {
  try {
    const createdSession = await createAssistantConversationSession(buildCreateSessionPayload())
...
const handleRenameSession = async (sessionOverride?: AssistantConversationSessionSummaryItem | AssistantConversationDetailItem | null) => {
  ...
  try {
    ...
    const renamed = await renameAssistantConversationSession(targetSession.id, { title: nextTitle })
...
const handleArchiveSession = async (sessionOverride?: AssistantConversationSessionSummaryItem | AssistantConversationDetailItem | null) => {
  const targetSession = sessionOverride ?? currentSessionDetail.value
  if (!targetSession) {
    return
  }
  try {
    ...
    await archiveAssistantConversationSession(targetSession.id)
...
const handleRestoreSession = async (sessionOverride?: AssistantConversationSessionSummaryItem | AssistantConversationDetailItem | null) => {
  const targetSession = sessionOverride ?? currentSessionDetail.value
  if (!targetSession) {
    return
  }
  try {
    const restored = await restoreAssistantConversationSession(targetSession.id)
    archivedView.value = false
...
const handleDeleteSession = async (sessionOverride?: AssistantConversationSessionSummaryItem | AssistantConversationDetailItem | null) => {
  const targetSession = sessionOverride ?? currentSessionDetail.value
  if (!targetSession) {
    return
  }
  try {
    ...
    await deleteAssistantConversationSession(targetSession.id)
    if (currentSessionDetail.value?.id === targetSession.id) {
      clearSelectedSession()
    }
    await loadSessionList(true)
...
const handleSessionCommand = async (
  session: AssistantConversationSessionSummaryItem,
  command: string
) => {
  if (command === 'rename') {
...
const formatReferenceTypeLabel = (type: string) => {
  ...
  return ASSISTANT_REFERENCE_TYPE_LABELS[normalizedType] || rawType.replace(/_/g, ' ')
}

const shouldShowReferenceLabel = (reference: AssistantReferenceItem) => {
  const label = formatReferenceTypeLabel(reference.type)
  const title = reference.title.trim()
  return Boolean(title) && title !== label
}

const formatReferenceDisplayText = (reference: AssistantReferenceItem) => {
  const label = formatReferenceTypeLabel(reference.type)
  const title = reference.title.trim()
 ...
 */
const actionKey = (action: AssistantActionItem, index: number) =>
  `${action.type}:${index}:${action.title}|${computeActionParamsHash(action.params as Record<string, unknown>)}`

/**
 * Assistant 只负责给出动作建议，真正写入仍走平台执行中心接口并在用户确认后发生。
 */
const executeAction = async (action: { type: string; title: string; description: string; requiresConfirm: boolean; params: Record<string, unknown> }, key: string) => {
  ...
  const markExecuted = () => {
    ...
    if (sessionId) {
      void markAssistantActionExecuted(sessionId, key).catch(() => {
  ...
  try {
    ...
    if (action.type === 'CREATE_WORK_ITEM_DRAFT') {
      ...
      const name = String(params.name || (content.slice(0, 40) || `GitPilot 创建的${workItemType}草稿`))
    ...
    if (action.type === 'CREATE_TEST_PLAN_DRAFT') {
      await createTestPlan({ name: String(params.name || 'GitPilot 测试计划草稿'), projectId: Number(params.projectId), iterationId: Number(params.iterationId), status: '草稿', description: String(params.description || ''), cases: [] })
      markExecuted()
...
const handleConfirmAction = async (action: AssistantActionItem, index: number) => executeAction(action, actionKey(action, index))

const updateMessage = (messageId: string, updater: (current: AssistantMessageItem) => AssistantMessageItem) => {
...
const buildPayload = (question: string, selection?: AssistantSelectionPayload | null, slashCommand?: string | null): AssistantSessionChatRequestPayload => ({
  question,
...
const resolveAssistantRenderableContent = (message: AssistantMessageItem) => {
  if (message.content) return message.content
  return '暂无内容'
}

const renderAssistantMessage = (message: AssistantMessageItem) => renderAssistantMarkdownToHtml(resolveAssistantRenderableContent(message), { thinkBlockKeyPrefix: message.id, isThinkBlockOpen: (thinkBlockKey: string) => Boolean(thinkBlockOpenState.get(thinkBlockKey)) })
const formatDebugInfo = (debug: AssistantDebugInfoItem | null) => JSON.stringify(debug || {}, null, 2)

/**
 * 流式等待阶段没有真实回答内容时，仅展示过程状态，避免伪造成可展开的思考过程。
 */
const shouldShowAssistantMarkdown = (message: AssistantMessageItem) => {
 ...
 */
const shouldShowInlineStreamStatus = (message: AssistantMessageItem) => {
  return message.status === 'streaming' && message.id === currentStreamingAssistantMessageId.value
}

const resolveMessageToolExecutionItems = (message: AssistantMessageItem): AssistantToolExecutionViewItem[] =>
  normalizeAssistantToolExecutions(message.toolExecutions || [])

const resolveMessageProcessTraceSummary = (message: AssistantMessageItem): AssistantToolTraceSummary =>
  buildAssistantToolTraceSummary(resolveMessageToolExecutionItems(message)) || {
    title: '工具调用',
    description: '',
    countText: '0 次',
    tone: 'muted'
...
const shouldShowProcessTrace = (message: AssistantMessageItem) =>
  message.role === 'assistant' && resolveMessageToolExecutionItems(message).length > 0

const resolveMessageToolExecutionHint = (message: AssistantMessageItem) => {
...
const resolveProcessToneIcon = (tone: AssistantToolTraceSummary['tone']) => {
...
const resolveDebugToolExecutions = (debug: AssistantDebugInfoItem | null | undefined): Array<Record<string, unknown>> =>
...
function applyStreamStatus(payload: AssistantStreamStatusEvent) {
 ...
 * 避免所有异常都落成“助手不可用”。
 */
function resolveAssistantStreamErrorMessage(message?: string) {
  const normalized = (message || '').trim()
  if (!normalized) {
    return 'GitPilot 本轮连接已中断，可直接重试；如果页面里已经出现确认卡片，也可以继续使用当前结果'
  }
...
const submitConversation = async (question: string, userContent: string, selection?: AssistantSelectionPayload | null, slashCommand?: string | null) => {
  ...
  // actionKey 已经带 paramsHash 不会串号，新轮次新动作会得到不同 key。
    currentStreamStatus.value = { stage: 'planning', message: 'GitPilot 正在分析问题' }
  ...
  try {
    ...
    const streamController = pendingFiles.value.length
      ? await streamAssistantSessionChatWithFiles(writableSessionId, payload, pendingFiles.value, {
          onStatus: (streamPayload: AssistantStreamStatusEvent) => { applyStreamStatus(streamPayload) },
          onMeta: (streamPayload: AssistantStreamMetaEvent) => { applyStreamDisplayState(streamPayload.references, streamPayload.suggestions, streamPayload.actions, streamPayload.selectionCards, streamPayload.debug) },
          onDelta: (streamPayload: AssistantStreamDeltaEvent) => queueStreamDelta(assistantMessageId, streamPayload.content || ''),
          onDone: (streamPayload: AssistantStreamDoneEvent) => {
        ...
        updateMessage(assistantMessageId, (current) => ({ ...current, content: shouldPreferTerminalContent ? (streamPayload.content || current.content) : resolveAssistantFinalContent(current.content, streamPayload.content), status: 'done', attachments: current.attachments || [], toolExecutions: resolveDebugToolExecutions(streamPayload.debug) }))
          ...
          onError: (streamPayload: AssistantStreamErrorEvent) => {
            if (stopRequested.value) {
              finishStream({ preserveStopRequested: true })
              return
            }
            flushPendingStreamDeltas(true)
            const errorMessage = resolveAssistantStreamErrorMessage(streamPayload.message) || currentStreamStatusText.value
            updateMessage(assistantMessageId, (current) => ({ ...current, content: errorMessage || current.content || 'GitPilot 本轮连接已中断，可直接重试', status: 'error', attachments: current.attachments || [], toolExecutions: current.toolExecutions || resolveDebugToolExecutions(currentDebug.value) }))
            finishStream()
            ElMessage.error(errorMessage || 'GitPilot 本轮连接已中断，可直接重试')
            void refreshCurrentSessionFromCloud()
          }
        })
      : await streamAssistantSessionChat(writableSessionId, payload, {
      onStatus: (payload: AssistantStreamStatusEvent) => { applyStreamStatus(payload) },
      onMeta: (payload: AssistantStreamMetaEvent) => { applyStreamDisplayState(payload.references, payload.suggestions, payload.actions, payload.selectionCards, payload.debug) },
      onDelta: (payload: AssistantStreamDeltaEvent) => queueStreamDelta(assistantMessageId, payload.content || ''),
      onDone: (payload: AssistantStreamDoneEvent) => {
        if (stopRequested.value) {
          finishStream({ preserveStopRequested: true })
          return
        }
      ...
      onError: (payload: AssistantStreamErrorEvent) => {
        if (stopRequested.value) {
          finishStream({ preserveStopRequested: true })
          return
        }
        flushPendingStreamDeltas(true)
        const errorMessage = resolveAssistantStreamErrorMessage(payload.message) || currentStreamStatusText.value
            updateMessage(assistantMessageId, (current) => ({ ...current, content: errorMessage || current.content || 'GitPilot 本轮连接已中断，可直接重试', status: 'error', attachments: current.attachments || [], toolExecutions: current.toolExecutions || resolveDebugToolExecutions(currentDebug.value) }))
        finishStream()
            ElMessage.error(errorMessage || 'GitPilot 本轮连接已中断，可直接重试')
        void refreshCurrentSessionFromCloud()
    ...
    const message = resolveAssistantStreamErrorMessage(error instanceof Error ? error.message : '')
...
const handleSelectOption = async (selectionCard: AssistantSelectionCardItem, option: AssistantSelectionOptionItem) => {
...
function applyStreamDisplayState(references: AssistantReferenceItem[], suggestions: string[], actions: AssistantActionItem[], selectionCards: AssistantSelectionCardItem[], debug: AssistantDebugInfoItem | null) {
...
async function restoreThinkBlocksAndScroll(shouldScroll = true) {
  ...
  if (messageScrollRef.value) {
    messageScrollRef.value.querySelectorAll<HTMLDetailsElement>('.assistant-think-block[data-think-key]').forEach((thinkBlock) => {
...
function applySessionDetail(detail: AssistantConversationDetailItem) {
  currentSessionDetail.value = detail
  const latestDisplayState = detail.latestDisplayState || emptyLatestDisplayState()
  const messages: AssistantMessageItem[] = detail.messages.map((message) => ({ id: `cloud-${message.id}`, role: message.role === 'user' ? 'user' : 'assistant', content: message.content || '', status: message.status === 'error' ? 'error' : 'done', attachments: message.attachments || [] }))
...
async function handleVoiceRecorderStop(sourceSampleRate: number) {
  ...
  try {
    ...
    if (!transcribedText) {
      throw new Error('GitPilot 未返回可用的转写文本')
    }
    if (assistantDrawerDisposed || !drawerVisible.value) {
      return
    }
    ...
    if (!assistantDrawerDisposed && drawerVisible.value) {
    ElMessage.error(error?.response?.data?.message || error?.message || 'GitPilot 语音转写失败')
    }
  } finally {
    transcribing.value = false
  }
...
function handleFileInputChange(event: Event) {
  ...
  if (files.length > 3) {
    ElMessage.warning('GitPilot 第一版每次最多上传 3 个文档')
  }
  input.value = ''
}
...
function handleDownloadAttachment(attachment: AssistantAttachmentItem) {
...
function toAttachmentSummary(file: File): AssistantAttachmentItem {
...
function patchCurrentSessionSummary(summary: AssistantConversationSessionSummaryItem) {
  sessionSummaries.value = mergeSessionSummaries([summary], sessionSummaries.value)
}

function mergeSessionSummaries(base: AssistantConversationSessionSummaryItem[], incoming: AssistantConversationSessionSummaryItem[]) {
  const seen = new Set<number>()
  return [...base, ...incoming].filter((item) => seen.has(item.id) ? false : (seen.add(item.id), true))
}

interface AssistantRouteContextSnapshot {
  routeName: string
  projectId: number | null
  taskId: number | null
  iterationId: number | null
...
/**
 * Assistant 会话会把页面锚点固化到服务端，因此打开抽屉时需要优先恢复与当前路由一致的会话，
...
function isSessionAlignedWithCurrentContext(session: Pick<AssistantConversationSessionSummaryItem, 'routeName' | 'projectId' | 'taskId' | 'iterationId' | 'planId' | 'wikiSpaceId' | 'wikiPageId'>) {
...
function buildCurrentRouteContext(): AssistantRouteContextSnapshot {
  return {
...
function buildCreateSessionPayload(): CreateAssistantConversationSessionPayload {
...
function formatSessionContext(session: Pick<AssistantConversationSessionSummaryItem, 'routeName' | 'projectId' | 'taskId' | 'iterationId' | 'planId' | 'wikiSpaceId' | 'wikiPageId'>) {
...
function formatSessionTime(session: AssistantConversationSessionSummaryItem) {
...
function readSelectedSessionId() {
  ...
  const parsed = Number(window.sessionStorage.getItem(ASSISTANT_SELECTED_SESSION_STORAGE_KEY))
  return Number.isNaN(parsed) || parsed <= 0 ? null : parsed
}

function persistSelectedSessionId(sessionId: number | null) {
  if (typeof window === 'undefined') return
  if (!sessionId) window.sessionStorage.removeItem(ASSISTANT_SELECTED_SESSION_STORAGE_KEY)
  else window.sessionStorage.setItem(ASSISTANT_SELECTED_SESSION_STORAGE_KEY, String(sessionId))
}
</script>
...
.assistant-thinking-indicator.compact {
  margin-top: 10px;
  padding-top: 10px;
...
.assistant-markdown-content :deep(h1),
.assistant-markdown-content :deep(h2),
.assistant-markdown-content :deep(h3),
.assistant-markdown-content :deep(h4) {
  margin: 0 0 12px;
  color: #0f172a;
  font-family: var(--app-font-heading);
  line-height: 1.45;
...
.assistant-markdown-content :deep(h1) { font-size: 20px; }
.assistant-markdown-content :deep(h2) { font-size: 18px; }
.assistant-markdown-content :deep(h3) { font-size: 16px; }
.assistant-markdown-content :deep(h4) { font-size: 15px; }

.assistant-markdown-content :deep(p) {
  margin: 0 0 10px;
}

.assistant-markdown-content :deep(h1:last-child),
.assistant-markdown-content :deep(h2:last-child),
.assistant-markdown-content :deep(h3:last-child),
.assistant-markdown-content :deep(h4:last-child),
.assistant-markdown-content :deep(p:last-child),
.assistant-markdown-content :deep(ul:last-child),
.assistant-markdown-content :deep(ol:last-child),
.assistant-markdown-content :deep(blockquote:last-child),
.assistant-markdown-content :deep(pre:last-child),
...
.assistant-markdown-content :deep(ul),
.assistant-markdown-content :deep(ol) {
  margin: 0 0 10px;
  padding-left: 20px;
}

.assistant-markdown-content :deep(ul ul),
.assistant-markdown-content :deep(ul ol),
.assistant-markdown-content :deep(ol ul),
.assistant-markdown-content :deep(ol ol) {
  margin-top: 6px;
  margin-bottom: 0;
}

.assistant-markdown-content :deep(li + li) {
  margin-top: 4px;
}

.assistant-markdown-content :deep(a) {
  color: var(--app-primary);
  text-decoration: underline;
  text-underline-offset: 2px;
}

.assistant-markdown-content :deep(blockquote) {
  margin: 0 0 10px;
  padding: 10px 12px;
...
.assistant-markdown-content :deep(blockquote p:last-child) {
  margin-bottom: 0;
}

.assistant-markdown-content :deep(code) {
  padding: 2px 6px;
  border-radius: 8px;
  background: rgba(var(--app-primary-rgb), 0.08);
  color: #0f172a;
...
.assistant-markdown-content :deep(pre) {
  margin: 0 0 10px;
  padding: 12px;
  overflow: auto;
  border-radius: 14px;
...
.assistant-markdown-content :deep(pre code) {
  padding: 0;
  background: transparent;
  color: inherit;
  font-size: 12px;
...
.assistant-markdown-content :deep(hr) {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid rgba(var(--app-outline-rgb), 0.12);
}

.assistant-markdown-content :deep(img) {
  display: block;
  max-width: 100%;
  margin: 0 0 10px;
  border-radius: 12px;
...
.assistant-markdown-content :deep(.assistant-table-wrap) {
  margin: 0 0 10px;
  overflow: auto;
...
.assistant-markdown-content :deep(.assistant-table-wrap table) {
  width: max-content;
  min-width: 100%;
  border-collapse: collapse;
}

.assistant-markdown-content :deep(.assistant-table-wrap th),
.assistant-markdown-content :deep(.assistant-table-wrap td) {
  padding: 10px 12px;
...
.assistant-markdown-content :deep(.assistant-table-wrap tbody tr:nth-child(even)) {
  background: rgba(248, 250, 252, 0.72);
}

.assistant-markdown-content :deep(.assistant-table-wrap tbody tr:last-child td) {
  border-bottom: 0;
}

.assistant-markdown-content :deep(.task-list-item) {
  list-style: none;
  margin-left: -20px;
}

.assistant-markdown-content :deep(.task-list-item-label) {
  display: inline-flex;
  align-items: flex-start;
  gap: 8px;
}

.assistant-markdown-content :deep(.task-list-item-checkbox) {
  margin-top: 4px;
  accent-color: var(--app-primary);
  pointer-events: none;
}

.assistant-markdown-content :deep(.assistant-think-block) {
  margin: 0 0 12px;
  border: 1px solid rgba(51, 65, 85, 0.12);
...
.assistant-markdown-content :deep(.assistant-think-block summary) {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
...
.assistant-markdown-content :deep(.assistant-think-summary-main) {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  min-width: 0;
...
.assistant-markdown-content :deep(.assistant-think-status-icon) {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 18px;
...
.assistant-markdown-content :deep(.assistant-think-status-icon.thinking) {
...
.assistant-markdown-content :deep(.assistant-think-block.is-done) {
  border-color: rgba(15, 118, 110, 0.2);
  border-left-color: #0f766e;
  background:
    linear-gradient(90deg, rgba(236, 253, 245, 0.96), rgba(255, 255, 255, 0.82)),
...
.assistant-markdown-content :deep(.assistant-think-block.is-done .assistant-think-summary-label) {
  color: #0f766e;
}

.assistant-markdown-content :deep(.assistant-think-summary-label) {
  letter-spacing: 0;
}

.assistant-markdown-content :deep(.assistant-think-dots) {
  display: inline-flex;
  align-items: flex-end;
  gap: 1px;
...
.assistant-markdown-content :deep(.assistant-think-dots span) {
  display: inline-block;
  min-width: 4px;
  animation: assistant-think-dot-bounce 1.1s ease-in-out infinite;
  transform-origin: center bottom;
}

.assistant-markdown-content :deep(.assistant-think-dots span:nth-child(2)) {
  animation-delay: 0.16s;
}

.assistant-markdown-content :deep(.assistant-think-dots span:nth-child(3)) {
  animation-delay: 0.32s;
}

.assistant-markdown-content :deep(.assistant-think-block summary::-webkit-details-marker) {
  display: none;
}

.assistant-markdown-content :deep(.assistant-think-block summary::after) {
  content: '展开';
  color: #64748b;
  font-size: 11px;
  font-weight: 700;
...
.assistant-markdown-content :deep(.assistant-think-block[open] summary::after) {
  content: '收起';
}

.assistant-markdown-content :deep(.assistant-think-content) {
...
.assistant-markdown-content :deep(.assistant-process-block .assistant-think-content p) {
  margin: 10px 0 0;
  color: #475569;
  font-size: 12px;
  line-height: 1.7;
...
@keyframes assistant-think-dot-bounce {
  0%,
  60%,
  100% {
    opacity: 0.35;
